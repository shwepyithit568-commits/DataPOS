...
<div class="w-full space-y-0.5 pb-6"
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-0.5 sm:gap-1">
        ...
        <div class="p-2 sm:p-2.5 bg-white dark:bg-slate-900 rounded border border-slate-200/90 dark:border-slate-800 shadow-2xs flex items-center justify-center gap-2.5 sm:gap-3 transition">
            ...
            <div class="min-w-0">
                ...
                <div class="text-xs sm:text-sm font-black font-mono text-slate-900 dark:text-slate-100 tabular-nums mt-0.5" x-text="rows.length + ' Items (' + fmtQty(totalQty) + ' Qty)'">
                </div>
            </div>
        </div>
        ...
        <div class="p-2 sm:p-2.5 bg-white dark:bg-slate-900 rounded border border-slate-200/90 dark:border-slate-800 shadow-2xs flex items-center justify-center gap-2.5 sm:gap-3 transition">
            ...
            <div class="min-w-0">
                ...
                <div class="text-xs sm:text-sm font-black font-mono text-slate-900 dark:text-slate-100 tabular-nums mt-0.5 truncate" x-text="fmt(subtotal)">
                </div>
            </div>
        </div>
        ...
        <div class="p-2 sm:p-2.5 bg-white dark:bg-slate-900 rounded border border-emerald-200/90 dark:border-emerald-900/50 shadow-2xs flex items-center justify-center gap-2.5 sm:gap-3 transition">
            ...
            <div class="min-w-0">
                ...
                <div class="text-xs sm:text-sm font-black font-mono text-emerald-600 dark:text-emerald-400 tabular-nums mt-0.5 truncate" x-text="fmt(netTotal)">
                </div>
            </div>
        </div>
    ...
    {{-- 3. Main Form Card --}}
    <form x-ref="poForm" method="POST" action="{{ url('/store/' . $store->slug . '/pos/purchases') }}" enctype="multipart/form-data"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-1.5 sm:gap-2 pt-1.5 border-t border-slate-200/60 dark:border-slate-700/60">
                ...
                <div>
                    <label for="po-discount" class="block text-xs font-bold text-rose-600 dark:text-rose-400 mb-0.5 flex items-center justify-between">
                        ...
                        <span class="text-[10px] font-normal text-slate-400">(-) နုတ်မည်</span>
                    </label>
        ...
        {{-- Section 2: Product Finder, Camera Scanner & Live Search --}}
        ...
        <div class="sticky bottom-0 z-20 bg-white/95 dark:bg-slate-900/95 backdrop-blur-xs border-t border-slate-200/90 dark:border-slate-800 p-2 sm:p-2.5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 shadow-lg">
            <div class="flex items-center gap-3 flex-wrap">
                <div>
                    ...
                    <span class="text-xs sm:text-sm font-bold text-slate-700 dark:text-slate-300 font-mono mt-0.5 block" x-text="fmt(subtotal)"></span>
                </div>
                <template x-if="discountAmount > 0">
                    <div class="border-l border-slate-200 dark:border-slate-700 pl-2.5">
                        ...
                        <span class="text-xs sm:text-sm font-bold text-rose-600 font-mono mt-0.5 block" x-text="'- ' + fmt(discountAmount)"></span>
                    </div>
                </template>
                <template x-if="deliveryFee > 0">
                    <div class="border-l border-slate-200 dark:border-slate-700 pl-2.5">
                        ...
                        <span class="text-xs sm:text-sm font-bold text-amber-600 font-mono mt-0.5 block" x-text="'+ ' + fmt(deliveryFee)"></span>
                    </div>
                </template>
                <div class="border-l border-slate-200 dark:border-slate-700 pl-2.5">
