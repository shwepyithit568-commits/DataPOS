@php
    ...
    $backUrl = route('pos.purchases.returns', ['store_slug' => $store->slug]);
@endphp
<!DOCTYPE html>
...
<head>
    ...
    <title>{{ __('messages.po_return_slip_title') }} - {{ $purchaseReturn->return_number }} - {{ $store->name }}</title>
...
<body>
        ...
        <div class="top-nav-inner">
            <div class="top-nav-left">
                ...
                <span class="slip-badge-bar">
                    {{ __('messages.po_return_slip_title') }}
                </span>
    ...
    <div class="preview-stage" id="previewStage">
        <div id="voucherPaper" class="voucher-paper paper-{{ $paperSize }}">
            ...
            <div class="meta-grid">
                <div class="meta-row">
                    <span class="meta-label">{{ __('messages.po_return_col_number') }}:</span>
                    <span class="meta-val font-mono font-bold">{{ $purchaseReturn->return_number }}</span>
                </div>
                <div class="meta-row">
                    <span class="meta-label">{{ __('messages.po_return_col_date') }}:</span>
                    <span class="meta-val font-mono">{{ $purchaseReturn->returned_at?->format('d M Y, H:i') ?? $purchaseReturn->created_at->format('d M Y, H:i') }}</span>
                </div>
                @if ($purchaseReturn->purchaseOrder)
                    <div class="meta-row">
                        <span class="meta-label">{{ __('messages.po_col_po_number') }}:</span>
                        <span class="meta-val font-mono">{{ $purchaseReturn->purchaseOrder->po_number }}</span>
                    </div>
                @endif
                @if ($purchaseReturn->supplier)
                    <div class="meta-row">
                        <span class="meta-label">{{ __('messages.supplier_col_name') }}:</span>
                        <span class="meta-val">{{ $purchaseReturn->supplier->name }}</span>
                    </div>
                    @if ($purchaseReturn->supplier->phone)
                        <div class="meta-row">
                            <span class="meta-label">Tel:</span>
                            <span class="meta-val font-mono">{{ $purchaseReturn->supplier->phone }}</span>
                        </div>
                    @endif
                @endif
                @if ($purchaseReturn->createdBy)
                    <div class="meta-row">
                        <span class="meta-label">{{ __('messages.user') }}:</span>
                        <span class="meta-val">{{ $purchaseReturn->createdBy->name }}</span>
                    </div>
                @endif
            </div>
            ...
            <table class="items-table">
                <thead>
                    <tr>
                        <th class="text-left" style="width: 50%;">{{ __('messages.products') }}</th>
                        <th class="text-right" style="width: 15%;">{{ __('messages.quantity') }}</th>
                        <th class="text-right" style="width: 17%;">{{ __('messages.cost') }}</th>
                        <th class="text-right" style="width: 18%;">{{ __('messages.receiving_total') }}</th>
                    </tr>
                </thead>
                <tbody>
            ...
            <div class="total-box">
                <div class="meta-row">
                    <span class="meta-label font-bold">{{ __('messages.po_returns_total_qty') }}:</span>
                    <span class="meta-val font-mono font-bold">{{ format_quantity($purchaseReturn->total_quantity, $store) }} units</span>
                </div>
                <div class="total-row" style="margin-top: 4px;">
                    <span>{{ __('messages.po_returns_total_val') }}:</span>
                    <span class="font-mono">{{ format_currency($purchaseReturn->total_cost, $store) }}</span>
                </div>
            </div>
            ...
            @if ($purchaseReturn->reason)
                <div class="reason-box">
                    <span class="font-bold">{{ __('messages.po_return_reason') }}:</span>
                    <span>{{ $purchaseReturn->reason }}</span>
                </div>
            @endif

            {{-- Authorized & Supplier Hand-over Signatures --}}
            <div class="signatures">
                <div class="sig-block">
                    {{ __('messages.po_return_authorized_by') }}
                </div>
                <div class="sig-block">
                    {{ __('messages.po_return_received_by') }}
                </div>
            </div>
    ...
    <script nonce="{{ $cspNonce ?? '' }}">
        ...
        var returnNumber = @js($purchaseReturn->return_number);
        ...
        var pdfConfig = {
            margin: [0, 0, 0, 0],
            filename: 'Purchase_Return_' + returnNumber + '.pdf',
        ...
        function setPaper(size) {
            ...
            // Save preference
            try {
                localStorage.setItem('purchase_return_paper_pref', size);
            } catch (e) {}
        ...
        async function shareJpgDirectly() {
            ...
            try {
                ...
                var fileName = 'Purchase_Return_' + returnNumber + '.png';
                    ...
                    if (isMobile && navigator.canShare && navigator.canShare({ files: [file] })) {
                        try {
                            await navigator.share({
                                title: 'Return #' + returnNumber,
                                text: 'Purchase Return #' + returnNumber + ' - ' + @js($store->name),
                                files: [file]
                            });
                            return;
                        } catch (shareErr) {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            var savedPref = localStorage.getItem('purchase_return_paper_pref');
            ...
            if (autoPrint) {
                var autoPref = localStorage.getItem('purchase_return_auto_print');
                ...
                autoPrint.addEventListener('change', function() {
                    localStorage.setItem('purchase_return_auto_print', this.checked ? 'true' : 'false');
                });
            }
