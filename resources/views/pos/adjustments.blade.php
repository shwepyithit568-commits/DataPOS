    ...
    <div class="space-y-6 pb-12" 
         x-data="{
             ...
             matchesFilter(itemNumber, submitter, reason, status) {
                 const matchesSearch = this.searchQuery === '' || 
                     itemNumber.toLowerCase().includes(this.searchQuery.toLowerCase()) ||
                     submitter.toLowerCase().includes(this.searchQuery.toLowerCase()) ||
                     reason.toLowerCase().includes(this.searchQuery.toLowerCase());
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 p-5 sm:p-6 rounded-2xl shadow-sm">
            <div>
                <div class="flex items-center gap-2.5">
                    <span class="inline-flex items-center justify-center p-2.5 rounded-xl bg-amber-500/10 text-amber-600 dark:text-amber-400 font-bold text-xl">
                        ⚡
                    </span>
                    <div>
            ...
            <div class="flex items-center gap-2.5 flex-wrap">
                <button type="button" @click="formModalOpen = true"
                        class="inline-flex items-center gap-2 rounded-xl px-5 py-2.5 text-xs sm:text-sm font-bold bg-amber-600 hover:bg-amber-500 text-white shadow-md shadow-amber-600/20 transition-all cursor-pointer">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
            <div class="rounded-2xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 p-4 sm:p-5 shadow-sm">
                ...
                <span class="text-[11px] text-slate-500 mt-0.5 block">Total Requests</span>
            </div>
            ...
            <div class="rounded-2xl bg-white dark:bg-slate-900 border border-emerald-200 dark:border-emerald-900/50 p-4 sm:p-5 shadow-sm">
                ...
                <span class="text-[11px] text-slate-500 mt-0.5 block">Approved & Posted</span>
            </div>

            <div class="rounded-2xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 p-4 sm:p-5 shadow-sm">
                <span class="text-[11px] font-bold uppercase tracking-wider text-slate-400 block">အသားတင် ပြင်ဆင်မှု Qty</span>
                <p class="text-2xl font-black font-mono mt-1 {{ $netQuantity < 0 ? 'text-rose-600' : ($netQuantity > 0 ? 'text-emerald-600' : 'text-slate-700 dark:text-slate-300') }}">
                    {{ $netQuantity > 0 ? '+' : '' }}{{ number_format($netQuantity, 2) }}
                </p>
                <span class="text-[11px] text-slate-500 mt-0.5 block">Net Stock Quantity</span>
            </div>
        </div>
        ...
        <div class="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 p-3 sm:p-4 rounded-2xl shadow-sm">
            <div class="relative flex-1 max-w-md">
                <input type="text" x-model="searchQuery"
                       placeholder="မှတ်တမ်းနံပါတ်၊ တင်သွင်းသူ သို့မဟုတ် အကြောင်းပြချက် ရှာရန်..."
                       class="w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 px-3.5 py-2 pl-9 text-xs sm:text-sm text-slate-900 dark:text-white placeholder-slate-400 focus:bg-white dark:focus:bg-slate-900 focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500">
            ...
            <div class="flex items-center gap-2 flex-wrap">
                ...
                <button type="button" @click="statusFilter = 'all'"
                        :class="statusFilter === 'all' ? 'bg-slate-900 text-white dark:bg-white dark:text-slate-900 font-bold' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 hover:bg-slate-200'"
                ...
                <button type="button" @click="statusFilter = 'pending'"
                        :class="statusFilter === 'pending' ? 'bg-amber-600 text-white font-bold' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 hover:bg-slate-200'"
                        class="px-3 py-1.5 rounded-xl text-xs transition font-semibold">
                    စောင့်ဆိုင်းဆဲ ({{ $pendingCount }})
                </button>
                <button type="button" @click="statusFilter = 'approved'"
                        :class="statusFilter === 'approved' ? 'bg-emerald-600 text-white font-bold' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 hover:bg-slate-200'"
                        class="px-3 py-1.5 rounded-xl text-xs transition font-semibold">
                    အတည်ပြုပြီး ({{ $approvedCount }})
                </button>
                <button type="button" @click="statusFilter = 'rejected'"
                        :class="statusFilter === 'rejected' ? 'bg-rose-600 text-white font-bold' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 hover:bg-slate-200'"
                        class="px-3 py-1.5 rounded-xl text-xs transition font-semibold">
                    ပယ်ချခဲ့သည်
                </button>
            </div>
        ...
        <div class="rounded-2xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 shadow-sm overflow-hidden">
            <div class="p-5 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
                <div>
                    <h2 class="text-base font-black text-slate-900 dark:text-white flex items-center gap-2">
                        <span>📜</span> စတော့ အတိုး/အလျော့ မှတ်တမ်းများ
                    </h2>
                    <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                        တင်သွင်းခဲ့သော တောင်းဆိုမှုများနှင့် မန်နေဂျာ စစ်ဆေးအတည်ပြုချက် စာရင်း
                    </p>
                </div>
            </div>
            ...
            @if ($requests->isEmpty())
                <div class="p-12 text-center text-slate-500">
                    ...
                    <button type="button" @click="formModalOpen = true"
                            class="mt-4 inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-amber-600 text-white text-xs font-bold shadow-sm">
                    ...
                    @foreach ($requests as $req)
                        ...
                        <div x-show="matchesFilter('{{ $req->adjustment_number }}', '{{ $req->submittedBy?->name ?? '' }}', '{{ addslashes($allReasons) }}', '{{ $req->status }}')"
                            ...
                            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                                <div>
                                    <div class="flex items-center gap-2.5 flex-wrap">
                                        <span class="font-mono font-black text-base text-slate-900 dark:text-white">{{ $req->adjustment_number }}</span>
                                ...
                                <div class="sm:text-right">
                                    <span class="text-xs text-slate-500 dark:text-slate-400 block">{{ __('messages.adjustment_net_change') }}</span>
                                    <span class="text-lg font-mono font-black {{ (float) $req->total_quantity < 0 ? 'text-rose-600 dark:text-rose-400' : ((float) $req->total_quantity > 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-400') }}">
                                ...
                                <table class="w-full text-xs">
                                    <thead class="bg-slate-100 dark:bg-slate-800/80 text-[11px] text-slate-500 dark:text-slate-400 font-bold uppercase tracking-wider">
                                        <tr>
                                            ...
                                            <th class="text-right px-3.5 py-2">ပြင်ဆင်သည့် အရေအတွက်</th>
                                            <th class="text-left px-3.5 py-2">အကြောင်းပြချက်</th>
                                        </tr>
                                    </thead>
                                    <tbody class="divide-y divide-slate-200/60 dark:divide-slate-800">
                                        @foreach ($req->items as $item)
                                            <tr>
                                                ...
                                                <td class="px-3.5 py-2.5 text-right font-mono font-black {{ (float) $item->quantity < 0 ? 'text-rose-600 dark:text-rose-400' : 'text-emerald-600 dark:text-emerald-400' }}">
                                                    {{ (float) $item->quantity > 0 ? '+' : '' }}{{ number_format((float) $item->quantity, 3) }}
                                                </td>
                                                <td class="px-3.5 py-2.5 text-slate-600 dark:text-slate-400">
                                                    <span class="inline-block px-2 py-0.5 rounded-md bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700">
                                                        {{ $item->reason }}
                                                    </span>
                                                </td>
                                            </tr>
            ...
            <div class="relative w-full max-w-3xl rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 shadow-2xl space-y-5 my-8 max-h-[90vh] overflow-y-auto"
                ...
                <div class="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-4">
                    <div class="flex items-center gap-2">
                        <span class="p-2 rounded-xl bg-amber-500/10 text-amber-600 font-bold">⚡</span>
                        <h3 class="text-lg font-black text-slate-900 dark:text-white">စတော့ အတိုး/အလျော့ အသစ် တင်သွင်းရန်</h3>
                    </div>
                    <button type="button" @click="formModalOpen = false" class="p-2 rounded-xl text-slate-400 hover:text-slate-600 hover:bg-slate-100 dark:hover:bg-slate-800">✕</button>
                </div>

                <form method="POST" action="{{ url('/store/' . $store->slug . '/pos/adjustments') }}" class="space-y-4">
                    @csrf
                    ...
                    <div class="space-y-3">
                        <template x-for="(r, i) in rows" :key="i">
                            <div class="rounded-2xl border border-slate-200 dark:border-slate-800 p-4 space-y-3 relative bg-slate-50/50 dark:bg-slate-950/40">
                                ...
                                <div class="relative">
                                    <label class="text-[11px] font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-1 block">ကုန်ပစ္စည်း အမည် သို့မဟုတ် Barcode / SKU</label>
                                    <input type="text" x-model="r.q" @input.debounce.250ms="searchProduct(r)" @focus="r.open = r.results.length > 0"
                                           ...
                                           class="w-full rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 px-3.5 py-2 text-sm font-semibold text-slate-900 dark:text-white placeholder-slate-400 focus:ring-2 focus:ring-amber-500/20">
                                    ...
                                    <div x-show="r.open" @click.outside="r.open = false" x-transition
                                        ...
                                        <template x-for="p in r.results" :key="p.id">
                                            <button type="button" @click="pickProduct(r, p)"
                                                    class="w-full text-left px-4 py-2.5 hover:bg-amber-500/10 flex items-center justify-between gap-3 transition">
                                                <div>
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-12 gap-3" x-show="r.name">
                                    <div class="sm:col-span-4 space-y-1">
                                        <label class="text-[11px] font-bold uppercase text-slate-500 block">အတိုး / အလျော့</label>
                                        <div class="flex rounded-xl bg-slate-200 dark:bg-slate-800 p-0.5">
                                            <button type="button" @click="r.dir = 'in'" class="flex-1 py-1 rounded-lg text-xs font-bold transition" :class="r.dir === 'in' ? 'bg-emerald-600 text-white shadow-sm' : 'text-slate-600 dark:text-slate-400'">+ အတိုး</button>
                                            <button type="button" @click="r.dir = 'out'" class="flex-1 py-1 rounded-lg text-xs font-bold transition" :class="r.dir === 'out' ? 'bg-rose-600 text-white shadow-sm' : 'text-slate-600 dark:text-slate-400'">− အလျော့</button>
                                        </div>
                                        <input type="hidden" :name="'items[' + i + '][quantity]'" :value="signed(r)" :disabled="!r.product_id">
                                    </div>
                                    <div class="sm:col-span-3 space-y-1">
                                        <label class="text-[11px] font-bold uppercase text-slate-500 block">အရေအတွက် (Qty)</label>
                                        <input type="number" min="0.001" step="any" x-model="r.quantity" :disabled="!r.product_id" class="w-full rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 px-3 py-1.5 text-right font-mono font-bold text-sm">
                                    </div>
                                    <div class="sm:col-span-5 space-y-1">
                                        <label class="text-[11px] font-bold uppercase text-slate-500 block">အကြောင်းပြချက်</label>
                                        <input type="text" :name="'items[' + i + '][reason]'" x-model="r.reason" :disabled="!r.product_id" maxlength="255" class="w-full rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 px-3 py-1.5 text-sm">
                                    </div>
                                </div>
                            </div>
                        </template>
                    ...
                    <button type="button" @click="addRow" class="w-full rounded-xl border-2 border-dashed border-slate-300 dark:border-slate-700 py-2.5 text-xs font-bold text-slate-500 hover:border-amber-500 hover:text-amber-600 transition">
                    ...
                    <div class="flex items-center justify-between p-3.5 rounded-2xl bg-slate-100 dark:bg-slate-800">
                        <div>
                            <span class="text-xs text-slate-500 block">စုစုပေါင်း ပြင်ဆင်မှု:</span>
                            <span class="text-base font-mono font-black" :class="totalQty < 0 ? 'text-rose-600' : 'text-emerald-600'" x-text="(totalQty > 0 ? '+' : '') + totalQty + ' Units'"></span>
                        </div>
                        <div class="flex gap-2">
                            ...
                            <button type="submit" :disabled="!valid" :class="valid ? 'bg-amber-600 hover:bg-amber-500 text-white cursor-pointer' : 'bg-slate-300 text-slate-500 cursor-not-allowed'" class="px-5 py-2 rounded-xl text-xs font-bold shadow-md transition">
                                တင်သွင်းမည်
                            </button>
                        </div>
                    </div>
