...
@section('content')
    ...
    <div class="mx-auto max-w-4xl px-4 py-6 space-y-6">
        ...
        <div class="flex items-center justify-between gap-3">
            <div>
                <p class="text-xs font-bold uppercase tracking-wide text-slate-400">{{ __('messages.opening_stock_title') }}</p>
                <h1 class="text-xl font-black mt-0.5">{{ __('messages.opening_stock_subtitle') }}</h1>
                <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">{{ __('messages.opening_stock_hint') }}</p>
            </div>
        ...
        <form method="POST" action="{{ url('/store/' . $store->slug . '/pos/opening-stock') }}"
                    ...
                    <div class="rounded-xl border border-slate-200 dark:border-slate-800 p-3 space-y-2 relative">
                        ...
                        <div class="relative">
                            <input type="text" x-model="r.q" @input.debounce.250ms="search(r)" @focus="r.open = r.results.length > 0"
                                   placeholder="{{ __('messages.opening_stock_product_placeholder') }}"
            ...
            <textarea name="notes" rows="2" maxlength="1000" placeholder="{{ __('messages.opening_stock_notes_placeholder') }}"
            ...
            <button type="submit" :disabled="!valid" :class="valid ? 'bg-sky-600 hover:bg-sky-500' : 'bg-slate-300 dark:bg-slate-700 cursor-not-allowed'"
                ...
                📋 {{ __('messages.opening_stock_submit') }}
            </button>
        </form>
        ...
        <div class="rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-5 shadow-sm space-y-4">
            ...
            @if ($requests->isEmpty())
                <p class="text-center text-sm text-slate-500 py-6">{{ __('messages.opening_stock_none') }}</p>
            @else
                @foreach ($requests as $req)
                    @php
                        ...
                        $statusLabel = ['pending' => __('messages.opening_stock_pending'), 'approved' => __('messages.opening_stock_approved'), 'rejected' => __('messages.opening_stock_rejected')][$req->status];
                    @endphp
                    <div class="rounded-xl border {{ $statusColors }} p-4 space-y-3">
                        <div class="flex items-center justify-between gap-3 flex-wrap">
                            <div>
                                <p class="font-mono font-bold">{{ $req->request_number }} · <span class="text-sm">{{ $statusLabel }}</span></p>
                                <p class="text-xs opacity-80">
                                    {{ __('messages.opening_stock_submitted_by') }}: {{ $req->submittedBy?->name ?? '—' }} · {{ $req->created_at->format('d M Y, H:i') }}
                                    @if ($req->reviewedBy) · {{ __('messages.opening_stock_reviewed_by') }}: {{ $req->reviewedBy->name }} @endif
                                </p>
                                @if ($req->review_notes)
                            ...
                            <div class="text-right">
                                <p class="text-xs opacity-80">{{ __('messages.reports_qty') }}: {{ number_format((float) $req->total_quantity, 3) }}</p>
                                <p class="font-black">Ks {{ number_format((float) $req->total_cost) }}</p>
                            </div>
                        </div>
                            ...
                            <table class="w-full text-sm">
                                <thead class="bg-slate-50 dark:bg-slate-800/60 text-xs text-slate-500 dark:text-slate-400">
                                    <tr>
                                        ...
                                        <th class="text-right px-3 py-2">{{ __('messages.opening_stock_on_hand') }}</th>
                                ...
                                <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                                    @foreach ($req->items as $item)
                                        <tr>
                                            ...
                                            <td class="px-3 py-2 text-right font-mono font-bold">{{ number_format((float) $item->quantity, 3) }}</td>
                                            <td class="px-3 py-2 text-right font-mono">Ks {{ number_format((float) $item->unit_cost, 2) }}</td>
                                            <td class="px-3 py-2 text-right font-mono">Ks {{ number_format((float) $item->line_total) }}</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
                            ...
                            <div class="flex gap-2 flex-wrap">
                                <form method="POST" action="{{ url('/store/' . $store->slug . '/pos/opening-stock/' . $req->id . '/approve') }}" class="flex-1 flex gap-2">
                                    ...
                                    <button class="rounded-lg px-4 py-2 text-sm font-bold bg-emerald-600 text-white hover:bg-emerald-500 transition">
                                        ✅ {{ __('messages.opening_stock_approve') }}
                                    </button>
                                </form>
                                <form method="POST" action="{{ url('/store/' . $store->slug . '/pos/opening-stock/' . $req->id . '/reject') }}">
                                    ...
                                    <button class="rounded-lg px-4 py-2 text-sm font-bold bg-rose-600 text-white hover:bg-rose-500 transition">
                                        ✕ {{ __('messages.opening_stock_reject') }}
                                    </button>
                                </form>
                            </div>
                        @elseif ($req->isPending())
                            <p class="text-xs font-semibold text-slate-500">{{ __('messages.opening_stock_waits') }}</p>
                        @endif
                    </div>
                @endforeach
            @endif
