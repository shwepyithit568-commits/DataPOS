{{-- Stock Transfers Listing --}}
@php
    $currentPage = $transfers->currentPage();
    $lastPage = $transfers->lastPage();
    $total = $transfers->total();
@endphp

<div class="mb-4">
    <x-admin.toolbar
        :sort-options="[
            ['value' => 'created_at', 'label' => __('messages.sort_by_date')],
            ['value' => 'transfer_number', 'label' => __('messages.sort_by_number')],
        ]"
        :current-sort="request('sort', 'created_at')"
        :current-direction="request('direction', 'desc')"
        :current-per-page="(int) request('per_page', 25)"
        :search-value="$search"
        :search-url="route('pos.transfers.index', $storeRouteParams)"
        search-key="search"
        :result-count="$total"
        view-mode="table"
        :show-filters="true"
        :show-import="false"
        :show-export="false"
        :show-add="false"
    />
</div>

<div class="flex items-center justify-between mb-4">
    <div class="flex items-center gap-2 border-b border-neutral-200 dark:border-white/10">
        <a href="{{ route('pos.transfers.index', $storeRouteParams) }}"
           class="px-4 py-2.5 text-sm font-medium border-b-2 border-blue-600 dark:border-emerald-400 text-blue-600 dark:text-emerald-400">
            🔄 {{ __('messages.sidebar_transfers') }}
            <span class="ml-1.5 inline-flex items-center justify-center min-w-5 h-5 px-1.5 text-xs font-medium rounded-full bg-neutral-100 text-neutral-600 dark:bg-white/10 dark:text-neutral-300">{{ $total }}</span>
        </a>
    </div>
    <a href="{{ route('pos.transfers.create', $storeRouteParams) }}"
       class="inline-flex items-center gap-1.5 px-3 h-8 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 dark:bg-emerald-600 dark:hover:bg-emerald-700 rounded-lg transition-colors">
        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4"/></svg>
        {{ __('messages.new_transfer') }}
    </a>
</div>

@if($transfers->isEmpty())
    <div class="text-center py-12 bg-white dark:bg-neutral-900 rounded-xl border border-neutral-200 dark:border-white/10">
        <div class="text-4xl mb-3">🔄</div>
        <p class="text-neutral-500 dark:text-neutral-400">{{ __('messages.no_transfers_found') }}</p>
        <p class="text-xs text-neutral-400 dark:text-neutral-500 mt-1">{{ __('messages.transfer_empty_hint') }}</p>
    </div>
@else
    <div class="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-white/10 rounded-xl overflow-hidden">
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b border-neutral-200 dark:border-white/10">
                        <th class="text-left px-4 py-3 font-medium text-neutral-500 dark:text-neutral-400">{{ __('messages.transfer_number') }}</th>
                        <th class="text-left px-4 py-3 font-medium text-neutral-500 dark:text-neutral-400">{{ __('messages.from') }}</th>
                        <th class="text-left px-4 py-3 font-medium text-neutral-500 dark:text-neutral-400">{{ __('messages.to') }}</th>
                        <th class="text-left px-4 py-3 font-medium text-neutral-500 dark:text-neutral-400">{{ __('messages.items') }}</th>
                        <th class="text-left px-4 py-3 font-medium text-neutral-500 dark:text-neutral-400">{{ __('messages.status') }}</th>
                        <th class="text-left px-4 py-3 font-medium text-neutral-500 dark:text-neutral-400">{{ __('messages.date') }}</th>
                        <th class="text-right px-4 py-3 font-medium text-neutral-500 dark:text-neutral-400">{{ __('messages.actions') }}</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach($transfers as $transfer)
                        <tr class="border-b border-neutral-100 dark:border-white/5 hover:bg-neutral-50 dark:hover:bg-white/[2%] transition-colors">
                            <td class="px-4 py-3 font-mono text-xs">
                                <a href="{{ route('pos.transfers.show', [...$storeRouteParams, 'transfer' => $transfer->id]) }}"
                                   class="text-blue-600 dark:text-emerald-400 hover:underline">{{ $transfer->transfer_number }}</a>
                            </td>
                            <td class="px-4 py-3 text-neutral-700 dark:text-neutral-300">{{ $transfer->fromWarehouse->name ?? '—' }}</td>
                            <td class="px-4 py-3 text-neutral-700 dark:text-neutral-300">{{ $transfer->toWarehouse->name ?? '—' }}</td>
                            <td class="px-4 py-3 text-neutral-600 dark:text-neutral-400">{{ $transfer->items->count() }}</td>
                            <td class="px-4 py-3">
                                @php
                                    $statusColors = [
                                        'pending' => 'bg-amber-100 text-amber-700 dark:bg-amber-500/20 dark:text-amber-300',
                                        'in_transit' => 'bg-blue-100 text-blue-700 dark:bg-blue-500/20 dark:text-blue-300',
                                        'completed' => 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-300',
                                        'cancelled' => 'bg-neutral-100 text-neutral-500 dark:bg-white/10 dark:text-neutral-400',
                                    ];
                                @endphp
                                <span class="inline-flex items-center px-2 py-0.5 text-xs rounded-full {{ $statusColors[$transfer->status] ?? '' }}">
                                    {{ __('messages.' . $transfer->status) }}
                                </span>
                            </td>
                            <td class="px-4 py-3 text-xs text-neutral-500 dark:text-neutral-400">{{ $transfer->created_at->format('d M Y') }}</td>
                            <td class="px-4 py-3 text-right">
                                <a href="{{ route('pos.transfers.show', [...$storeRouteParams, 'transfer' => $transfer->id]) }}"
                                   class="inline-flex items-center justify-center w-8 h-8 text-neutral-500 hover:bg-neutral-100 dark:hover:bg-white/5 rounded-lg transition-colors" title="{{ __('messages.view') }}">
                                    👁️
                                </a>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
        </div>
    </div>

    <div class="mt-4">
        {{ $transfers->withQueryString()->links('pagination::tailwind') }}
    </div>
@endif
