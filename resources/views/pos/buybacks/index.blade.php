...
@section('title', __('messages.sidebar_buy_back') . ' - ' . ($store->name ?? 'DataPOS'))
@section('main_padding', 'p-0.5 sm:p-1')

@section('content')
@php
    $totalCount = $buybacks->total();
@endphp
<div class="w-full space-y-0.5 pb-6"
     x-data="{
        viewMode: localStorage.getItem('pos_buybacks_view') || 'table',
        setView(mode) {
            this.viewMode = mode;
    ...
    {{-- ============================================================
         ...
         ============================================================ --}}
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-0.5 sm:gap-1" role="list" aria-label="{{ __('messages.sidebar_buy_back') }} Summary">
        {{-- Total Buy-Backs --}}
        <div role="listitem" class="bg-white dark:bg-slate-900 px-3 py-1.5 rounded-lg border border-slate-200/80 dark:border-slate-800 shadow-2xs flex items-center justify-center gap-2.5 sm:gap-3">
            ...
            <div class="min-w-0">
                ...
                <p class="text-[9px] sm:text-[10px] text-slate-500 dark:text-slate-400 mt-0.5 truncate font-bold uppercase tracking-wider">
                    {{ __('messages.total_buybacks') ?? 'စုစုပေါင်း အဝယ်တော်' }}
                </p>
            </div>
        </div>
    ...
    <div class="bg-white dark:bg-slate-900 px-2.5 py-1 rounded-lg border border-slate-200/80 dark:border-slate-800 shadow-2xs flex flex-col md:flex-row md:items-center md:justify-between gap-1">
        ...
        <div class="flex flex-wrap items-center gap-1.5 flex-1">
            {{-- Search Bar --}}
            <form method="GET" action="{{ route('pos.buybacks.index', $storeRouteParams) }}" class="relative min-w-[180px] sm:min-w-[260px] flex-1 max-w-sm">
                @if(!empty($status))
                ...
                <input type="text"
                       name="search"
                       value="{{ $search }}"
                       placeholder="{{ __('messages.search_buyback_placeholder') ?? 'Search by number, reason, customer...' }}"
            ...
            <div class="flex items-center gap-0.5 bg-slate-100 dark:bg-slate-800/80 p-0.5 rounded-md border border-slate-200/60 dark:border-slate-700">
                <a href="{{ route('pos.buybacks.index', array_merge($storeRouteParams, array_filter(['search' => $search]))) }}"
                   class="px-2 py-0.5 rounded text-[11px] font-bold transition cursor-pointer {{ empty($status) ? 'bg-white dark:bg-slate-700 text-slate-900 dark:text-white shadow-2xs' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200' }}">
                    {{ __('messages.all') ?? 'All' }} ({{ $summary['total'] }})
                </a>
                <a href="{{ route('pos.buybacks.index', array_merge($storeRouteParams, ['status' => 'pending'], array_filter(['search' => $search]))) }}"
                   class="px-2 py-0.5 rounded text-[11px] font-bold transition cursor-pointer {{ $status === 'pending' ? 'bg-amber-50 dark:bg-amber-950/60 text-amber-700 dark:text-amber-300 shadow-2xs' : 'text-slate-500 dark:text-slate-400 hover:text-amber-600' }}">
                    {{ __('messages.pending') }} ({{ $summary['pending'] }})
                </a>
                <a href="{{ route('pos.buybacks.index', array_merge($storeRouteParams, ['status' => 'completed'], array_filter(['search' => $search]))) }}"
                   class="px-2 py-0.5 rounded text-[11px] font-bold transition cursor-pointer {{ $status === 'completed' ? 'bg-emerald-50 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-300 shadow-2xs' : 'text-slate-500 dark:text-slate-400 hover:text-emerald-600' }}">
                    {{ __('messages.completed') }} ({{ $summary['completed'] }})
                </a>
                @if($summary['cancelled'] > 0)
                    <a href="{{ route('pos.buybacks.index', array_merge($storeRouteParams, ['status' => 'cancelled'], array_filter(['search' => $search]))) }}"
                       class="px-2 py-0.5 rounded text-[11px] font-bold transition cursor-pointer {{ $status === 'cancelled' ? 'bg-slate-200 dark:bg-slate-600 text-slate-900 dark:text-white shadow-2xs' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700' }}">
                        {{ __('messages.cancelled') }} ({{ $summary['cancelled'] }})
                    </a>
                @endif
            </div>
        </div>
    ...
    @if ($buybacks->isEmpty())
        ...
        <div class="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-lg p-6 sm:p-8 text-center shadow-2xs">
            ...
            <p class="text-xs sm:text-sm font-black text-slate-800 dark:text-slate-200">{{ __('messages.no_buybacks_found') }}</p>
            <p class="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5 max-w-sm mx-auto">{{ __('messages.buyback_empty_hint') }}</p>
            <a href="{{ route('pos.buybacks.create', $storeRouteParams) }}" class="h-7 px-3 rounded-md bg-sky-600 hover:bg-sky-500 text-white text-xs font-black shadow-2xs transition inline-flex items-center gap-1.5 mt-3 active:scale-95 cursor-pointer">
                <span>{{ __('messages.new_buyback') }}</span>
            </a>
        </div>
    @else
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left text-xs text-slate-600 dark:text-slate-300">
                    <thead class="bg-slate-50 dark:bg-slate-800/80 border-b border-slate-200 dark:border-slate-700 text-[11px] font-black uppercase tracking-wider text-slate-500 dark:text-slate-400">
                        <tr>
                            <th class="px-3 py-2 min-w-[150px]">{{ __('messages.buyback_number') }}</th>
                            <th class="px-3 py-2 min-w-[140px]">{{ __('messages.customer') }}</th>
                            <th class="px-3 py-2 min-w-[80px]">{{ __('messages.items') }}</th>
                            <th class="px-3 py-2 min-w-[130px] text-right">{{ __('messages.total_value') }}</th>
                            <th class="px-3 py-2 min-w-[110px]">{{ __('messages.status') }}</th>
                            ...
                            <tr class="hover:bg-slate-50/80 dark:hover:bg-slate-800/50 transition">
                                <td class="px-3 py-2 align-middle">
                                    <a href="{{ route('pos.buybacks.show', [...$storeRouteParams, 'buyback' => $buyback->id]) }}"
                                       class="font-mono text-xs font-black text-sky-600 dark:text-sky-400 hover:underline">
                                        {{ $buyback->buyback_number }}
                                    </a>
                                </td>
                                <td class="px-3 py-2 align-middle">
                                    <div class="flex items-center gap-1.5">
                                        <span class="text-xs font-semibold text-slate-800 dark:text-slate-200">{{ $buyback->customer?->name ?? 'Walk-in Customer' }}</span>
                                    </div>
                                </td>
                ...
                <div class="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-lg p-2.5 shadow-2xs hover:shadow-xs transition flex flex-col justify-between">
                    <div>
                        <div class="flex items-center justify-between gap-1.5">
                            <span class="font-mono text-xs font-black text-sky-600 dark:text-sky-400">{{ $buyback->buyback_number }}</span>
                            @php
                                $statusStyles = [
                                    'pending' => 'bg-amber-50 dark:bg-amber-950/60 text-amber-700 dark:text-amber-300 border border-amber-200 dark:border-amber-800/60',
                                    'completed' => 'bg-emerald-50 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800/60',
                        ...
                        <div class="mt-1.5 flex items-center justify-between gap-1.5 text-xs">
                            <div class="min-w-0">
                                ...
                                <p class="text-xs font-semibold text-slate-700 dark:text-slate-300 truncate">{{ $buyback->customer?->name ?? 'Walk-in' }}</p>
                            </div>
