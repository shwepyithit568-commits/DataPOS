                ...
                <table class="w-full min-w-[700px] text-left text-sm text-gray-600 dark:text-slate-300">
                    <thead class="bg-gray-50 dark:bg-slate-900/50 border-b dark:border-slate-700 font-semibold text-gray-700 dark:text-slate-200">
                        <tr>
                            ...
                            <th class="p-3">{{ __('messages.supplier_col_contact') }}</th>
                        ...
                        @forelse ($suppliers as $supplier)
                            <tr id="supplier-row-{{ $supplier->id }}"
                                ...
                                <td class="p-3">
                                    ...
                                    @if ($supplier->email)
                                        <div class="text-xs text-gray-400 dark:text-slate-500 mt-0.5">{{ $supplier->email }}</div>
                                    @endif
                                    ...
                                    <div class="flex items-center justify-end gap-1.5 whitespace-nowrap">
                                        ...
                                        @if ($supplier->purchase_orders_count > 0)
                                            <span class="min-h-11 inline-flex items-center gap-1.5 px-2.5 rounded-lg bg-gray-100 dark:bg-slate-700 text-gray-500 dark:text-slate-400 text-xs"
                                                ...
                                                {{ __('messages.supplier_used_by', ['count' => $supplier->purchase_orders_count]) }}
                                            </span>
                                        @else
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="p-8 text-center text-gray-400 dark:text-slate-500">
                                    ...
                                    <div class="text-xs text-gray-500 dark:text-slate-400">{{ __('messages.supplier_search_placeholder') }}</div>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
            ...
            <div x-show="viewMode === 'card'" x-cloak class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2">
                @forelse ($suppliers as $supplier)
                    <div id="supplier-card-{{ $supplier->id }}" data-supplier-row="{{ $supplier->id }}"
                        class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 overflow-hidden transition-colors duration-200 hover:shadow-md {{ $highlightSupplier && (int) $highlightSupplier === (int) $supplier->id ? 'ring-2 ring-violet-400/70' : '' }}">
                        {{-- Card header --}}
                        <div class="p-3 sm:p-4">
                            <div class="h-14 w-full rounded-md border dark:border-slate-600 bg-orange-50 dark:bg-orange-950/20 mb-2 flex items-center justify-center">
                                <span class="text-2xl">🏭</span>
                            </div>
                            <div class="font-bold text-gray-900 dark:text-slate-100 text-sm break-words" title="{{ $supplier->name }}">{{ $supplier->name }}</div>
                            @if ($supplier->email)
                                <div class="text-[11px] text-gray-400 dark:text-slate-500 truncate">{{ $supplier->email }}</div>
                            @endif
                            @if ($supplier->phone)
                                <div class="text-[11px] font-mono text-gray-400 dark:text-slate-500 mt-0.5">{{ $supplier->phone }}</div>
                            @endif
            ...
            <form x-ref="createForm" method="POST" action="{{ url('/store/' . $store->slug . '/admin/suppliers') }}"
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input id="add-supplier-phone" type="text" name="phone" value="{{ old('phone') }}"
                ...
                <div>
                    ...
                    <textarea id="add-supplier-notes" name="notes" rows="2"
                        ...
                        class="w-full border dark:border-slate-600 rounded-lg px-3 py-2.5 text-sm bg-white dark:bg-slate-900 text-gray-900 dark:text-slate-100 focus:ring-2 focus:ring-violet-500 focus:border-violet-500 outline-none transition resize-none">{{ old('notes') }}</textarea>
                </div>
