...
<div class="w-full space-y-2 sm:space-y-2.5"
    ...
    <div class="p-2.5 sm:p-3 bg-white dark:bg-slate-900 rounded-lg border border-slate-200/80 dark:border-slate-800 shadow-2xs flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2.5">
        ...
        <div class="flex items-center gap-2 flex-wrap shrink-0">
            ...
            <button type="button" @click="openCreate()"
                    class="px-3.5 py-1.5 rounded-lg text-xs font-black bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-500 hover:to-indigo-500 text-white shadow-md shadow-violet-900/20 transition flex items-center gap-1.5 active:scale-95 cursor-pointer">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-2.5">
        ...
        <div class="p-2.5 sm:p-3 bg-white dark:bg-slate-900 rounded-lg border border-slate-200/80 dark:border-slate-800 shadow-2xs flex flex-col justify-between">
            <div class="flex items-center justify-between">
                <span class="text-[10px] sm:text-[11px] font-bold uppercase tracking-wider text-slate-400">{{ __('messages.sidebar_suppliers') }}</span>
            ...
            <div class="mt-1">
                <p class="text-base sm:text-lg font-black text-slate-900 dark:text-slate-100 font-mono">{{ number_format($stats['total']) }}</p>
                <span class="text-[10px] text-slate-400 block mt-0.5">Total Suppliers</span>
            </div>
        </div>
        ...
        <div class="p-2.5 sm:p-3 bg-white dark:bg-slate-900 rounded-lg border border-amber-200/80 dark:border-amber-900/50 shadow-2xs flex flex-col justify-between">
            <div class="flex items-center justify-between">
                <span class="text-[10px] sm:text-[11px] font-bold uppercase tracking-wider text-amber-600 dark:text-amber-400">{{ __('messages.supplier_has_balance') }}</span>
                <span class="w-6 h-6 rounded bg-amber-50 dark:bg-amber-950/60 text-amber-600 dark:text-amber-400 grid place-items-center text-xs">⚠️</span>
            </div>
            <div class="mt-1">
                <p class="text-base sm:text-lg font-black text-amber-600 dark:text-amber-400 font-mono">{{ number_format($stats['owing']) }}</p>
                <span class="text-[10px] text-slate-400 block mt-0.5">Suppliers with Balance</span>
            </div>
        </div>
        ...
        <div class="p-2.5 sm:p-3 bg-white dark:bg-slate-900 rounded-lg border border-slate-200/80 dark:border-slate-800 shadow-2xs flex flex-col justify-between">
            <div class="flex items-center justify-between">
                <span class="text-[10px] sm:text-[11px] font-bold uppercase tracking-wider text-slate-400">{{ __('messages.supplier_col_outstanding') }}</span>
                <span class="w-6 h-6 rounded bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 grid place-items-center text-xs">💰</span>
            </div>
            <div class="mt-1">
                ...
                <span class="text-[10px] text-slate-400 block mt-0.5">Total Payables</span>
            </div>
        </div>
        ...
        <div class="p-2.5 sm:p-3 bg-white dark:bg-slate-900 rounded-lg border border-emerald-200/80 dark:border-emerald-900/50 shadow-2xs flex flex-col justify-between">
            <div class="flex items-center justify-between">
                <span class="text-[10px] sm:text-[11px] font-bold uppercase tracking-wider text-emerald-600 dark:text-emerald-400">Zero Balance</span>
            ...
            <div class="mt-1">
                <p class="text-base sm:text-lg font-black text-emerald-600 dark:text-emerald-400 font-mono">
                    {{ number_format(max(0, $stats['total'] - $stats['owing'])) }}
                </p>
                <span class="text-[10px] text-slate-400 block mt-0.5">Settled Suppliers</span>
            </div>
        </div>
    </div>
    ...
    <div x-show="viewMode === 'table'" class="w-full bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 rounded-lg shadow-2xs overflow-hidden transition">
            ...
            <table class="w-full text-left text-xs border-collapse font-sans text-slate-700 dark:text-slate-200">
                <thead class="sticky top-0 z-20 bg-slate-100 dark:bg-slate-800/95 backdrop-blur-xs border-b-2 border-slate-300 dark:border-slate-600 shadow-2xs select-none">
                    <tr class="text-[11px] font-black text-slate-700 dark:text-slate-200 uppercase tracking-wider divide-x divide-slate-300 dark:divide-slate-700">
                        <th class="py-2.5 px-3 min-w-[200px]">{{ __('messages.supplier_col_name') }}</th>
                        <th class="py-2.5 px-3 min-w-[140px]">{{ __('messages.supplier_col_phone') }}</th>
                        <th class="py-2.5 px-3 hidden md:table-cell min-w-[140px]">{{ __('messages.supplier_col_contact') }}</th>
                        <th class="py-2.5 px-3 text-center w-20">PO</th>
                        <th class="py-2.5 px-3 text-right min-w-[140px]">{{ __('messages.supplier_col_outstanding') }}</th>
                        <th class="py-2.5 px-3 text-right w-36">{{ __('messages.supplier_col_actions') }}</th>
                    </tr>
                </thead>
                        ...
                        <tr id="supplier-row-{{ $supplier->id }}"
                            ...
                            <td class="py-2.5 px-3">
                                <div class="flex items-center gap-2.5 min-w-0">
                                    ...
                                    <div class="min-w-0">
                                        <span class="font-bold text-slate-900 dark:text-slate-100 text-xs sm:text-sm block truncate" title="{{ $supplier->name }}">
                                            {{ $supplier->name }}
                                        </span>
                                        @if ($supplier->email)
                            ...
                            <td class="py-2.5 px-3 text-center">
                                @if ($supplier->purchase_orders_count > 0)
                                    <span class="inline-block px-2 py-0.5 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 text-xs font-bold font-mono">
                                ...
                                <div class="inline-flex items-center gap-1">
                                    ...
                                    @else
                                        <button type="button" @click="openConfirm({{ Js::from($supplier) }})"
                                                class="px-2 py-1 rounded text-xs font-bold bg-rose-50 hover:bg-rose-100 dark:bg-rose-950/40 text-rose-600 dark:text-rose-400 transition active:scale-95"
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="p-8 text-center text-slate-400">
                                ...
                                <button type="button" @click="openCreate()"
                                        class="mt-2 inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg text-xs font-bold text-white bg-violet-600 hover:bg-violet-700 transition shadow-sm">
        ...
        @if ($suppliers->hasPages())
            <div class="p-3 border-t border-slate-100 dark:border-slate-800 text-xs">{{ $suppliers->links() }}</div>
        @endif
    </div>
        ...
        @forelse ($suppliers as $supplier)
            <div id="supplier-card-{{ $supplier->id }}"
                ...
                <div class="p-3 space-y-2.5">
                    ...
                    <div class="flex items-start gap-2.5 border-b border-slate-100 dark:border-slate-800 pb-2">
                        <div class="shrink-0 w-9 h-9 rounded-full bg-gradient-to-br from-amber-500 to-orange-600 text-white grid place-items-center font-black text-xs select-none shadow-2xs">
                            {{ mb_strtoupper(mb_substr(trim($supplier->name), 0, 1)) }}
                        </div>
                ...
                <div class="p-2.5 bg-slate-50/80 dark:bg-slate-800/40 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between gap-1">
                    ...
                    @if ($supplier->purchase_orders_count > 0)
                        <span class="px-2 py-1.5 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-400 text-xs font-bold"
                              title="{{ __('messages.supplier_delete_blocked', ['count' => $supplier->purchase_orders_count]) }}">
                            🔒 In Use
                        </span>
                    @else
                        <button type="button" @click="openConfirm({{ Js::from($supplier) }})"
                                class="px-2 py-1.5 rounded-lg bg-rose-50 hover:bg-rose-100 dark:bg-rose-950/40 text-rose-600 dark:text-rose-400 text-xs font-bold transition active:scale-95"
                                title="{{ __('messages.delete') }}">
                            🗑️ Delete
                        </button>
                    @endif
    ...
    @if ($suppliers->hasPages())
        <div x-show="viewMode === 'card' || viewMode === 'cards'" class="p-3 bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-lg text-xs">
            {{ $suppliers->links() }}
        </div>
    @endif
                ...
                <form method="POST"
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div>
                            ...
                            <input type="tel" name="phone" x-model="formPhone" maxlength="50"
                                   ...
                                   class="w-full rounded-lg border border-slate-200 dark:border-slate-700 px-3 py-2 text-xs font-mono bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-slate-100 outline-none focus:ring-2 focus:ring-violet-500" />
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div>
                            ...
                            <input type="text" name="contact_person" x-model="formContactPerson" maxlength="100"
                                   ...
                                   class="w-full rounded-lg border border-slate-200 dark:border-slate-700 px-3 py-2 text-xs bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-slate-100 outline-none focus:ring-2 focus:ring-violet-500" />
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">
                    ...
                    <div>
                        ...
                        <textarea name="notes" x-model="formNotes" rows="2" maxlength="1000"
                                  ...
                                  class="w-full rounded-lg border border-slate-200 dark:border-slate-700 px-3 py-2 text-xs bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-slate-100 outline-none focus:ring-2 focus:ring-violet-500 resize-none"></textarea>
                    </div>
        ...
        <div class="min-h-full flex items-center justify-center p-4">
            <div class="relative w-full max-w-sm bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-2xl p-5 space-y-4" @click.stop>
                <div class="text-center space-y-2">
                    ...
                    <h4 class="text-sm font-black text-slate-900 dark:text-slate-100">{{ __('messages.supplier_delete_title') }}</h4>
