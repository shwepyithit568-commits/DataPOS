...
<div class="space-y-6 max-w-5xl">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <h1 class="text-2xl font-bold text-gray-900 dark:text-slate-100 font-outfit">Import Products</h1>
        <div class="w-full sm:w-auto flex flex-nowrap items-center gap-2 overflow-x-auto pb-1 -mx-1 px-1 sm:flex-wrap sm:overflow-visible sm:mx-0 sm:px-0 sm:pb-0">
            <a href="{{ route('store.admin.products.import.template', ['store_slug' => $store->slug]) }}" class="shrink-0 px-3 py-2 bg-green-600 text-white rounded-md text-xs font-semibold hover:bg-green-700 whitespace-nowrap">Download Product Template</a>
            <a href="{{ route('store.admin.products.index', ['store_slug' => $store->slug]) }}" class="shrink-0 text-xs text-violet-600 dark:text-violet-400 font-semibold hover:underline whitespace-nowrap">&larr; Back to Products</a>
        </div>
    </div>
                ...
                <details>
                    ...
                    <div class="mt-2 space-y-1 text-xs bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-800 rounded p-2 max-h-48 overflow-y-auto">
                        @foreach ($preview['failed_rows'] as $fr)
                            <div>Row {{ $fr['row'] }}{{ isset($fr['sku']) ? ' [SKU: ' . $fr['sku'] . ']' : '' }}: {{ $fr['reason'] }}</div>
                        @endforeach
                    </div>
                </details>
            @endif

            <form method="POST" action="{{ route('store.admin.products.import.confirm', ['store_slug' => $store->slug]) }}" class="flex flex-nowrap items-end gap-3 overflow-x-auto pb-1 -mx-1 px-1 sm:overflow-visible sm:mx-0 sm:px-0 sm:pb-0">
                @csrf
                <input type="hidden" name="token" value="{{ $preview['token'] }}">
                <div class="shrink-0">
                    <label class="block text-xs font-semibold text-gray-700 dark:text-slate-300 mb-1">Duplicate SKU Handling</label>
                    <select name="duplicate_strategy" class="border dark:border-slate-600 rounded px-3 py-2 text-sm bg-white dark:bg-slate-900">
                        <option value="skip" {{ $preview['duplicate_strategy'] === 'skip' ? 'selected' : '' }}>Skip duplicate products</option>
                        <option value="update" {{ $preview['duplicate_strategy'] === 'update' ? 'selected' : '' }}>Update existing products</option>
                    </select>
                </div>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl p-6 shadow-sm space-y-5 transition-colors duration-200">
        <form action="{{ route('store.admin.products.import', ['store_slug' => $store->slug]) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            ...
            <div>
                <label class="block text-sm font-semibold text-gray-700 dark:text-slate-300 mb-1">Duplicate SKU Handling</label>
                <select name="duplicate_strategy" class="border dark:border-slate-600 rounded px-3 py-2 text-sm bg-white dark:bg-slate-900">
                    <option value="skip">Skip duplicate products (default)</option>
        ...
        <div class="border-t dark:border-slate-700 pt-4">
            ...
            <p class="text-xs text-gray-400 dark:text-slate-500 mt-2">
                POS / AppSheet export formats are mapped automatically: Product_ID → sku, Product_Name → name,
                Sale_Price → retail_price, Warranty_Period → warranty. When the file has no stock_status column,
                Current_Stock (quantity) decides it — positive = in stock, 0/negative = out of stock. A product
                with a 0 or empty Sale_Price is always imported as out of stock. "parent_category" is the Main
                Category the "category" (sub-category) belongs under — blank means top-level.
            </p>
            <p class="text-xs text-gray-400 dark:text-slate-500 mt-2">
                <strong class="text-gray-600 dark:text-slate-300">Variants (စျေးမတူတဲ့ အရွယ်/အရောင်/connector တွေ):</strong>
                the <code>variants</code> column holds a JSON array — every variant has its own sku, retail_price,
                wholesale_price and stock_status, so Type C / Micro / Lightning (or 128GB / 256GB) with different
                prices are fully supported. Example:
                <code>[{"name":"Type C","sku":"L2009-TC","retail_price":5000,"wholesale_price":4500,"stock_status":"in_stock"},{"name":"Micro","sku":"L2009-MC","retail_price":4000,"wholesale_price":3700,"stock_status":"in_stock"},{"name":"Lightning","sku":"L2009-LT","retail_price":7000,"wholesale_price":6300,"stock_status":"in_stock"}]</code>
                Each variant may also carry an <code>attributes</code> array of {"label","value"} pairs — those rows
                appear on the product page's Specifications tab (e.g. Battery: 300mAh, Water Resistance: IP68).
                In the admin form, use <strong>Variant Presets</strong> to auto-generate these rows with one click.
            </p>
        </div>
    </div>

    <div class="bg-white dark:bg-slate-800 rounded-xl p-6 shadow-sm space-y-3">
        <h2 class="text-lg font-bold text-gray-900 dark:text-slate-100">Recent Product Imports</h2>
