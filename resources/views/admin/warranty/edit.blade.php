    ...
    <form method="POST" action="{{ route('store.admin.warranty.update', ['store_slug' => $store->slug, 'warranty' => $warranty->id]) }}" class="space-y-2 sm:space-y-2.5">
        ...
        <div class="sticky bottom-0 z-20 w-full border border-slate-200/90 bg-white/95 px-3 py-2 sm:px-4 backdrop-blur-md shadow-[0_-4px_16px_rgba(15,23,42,0.06)] dark:border-slate-800/90 dark:bg-slate-900/95 rounded-lg flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2 sm:gap-3">
            ...
            <div class="w-full sm:w-auto flex items-center gap-2">
                <a href="{{ route('store.admin.warranty.show', ['store_slug' => $store->slug, 'warranty' => $warranty->id]) }}"
                   class="sf-btn-3d flex-1 sm:flex-none justify-center px-3.5 py-2 text-xs font-bold transition text-center cursor-pointer">
                    {{ __('messages.cancel') }}
                </a>
                <button type="submit"
                        class="sf-btn-3d-primary flex-1 sm:flex-none justify-center px-5 py-2 text-white font-black text-xs transition flex items-center gap-1.5 cursor-pointer">
                    <span>💾</span>
