<div class="p-4 sm:p-6 space-y-6"
    ...
    <section class="space-y-3">
        ...
        <div x-show="showPayForm" x-cloak x-transition class="rounded-2xl border border-violet-200 bg-violet-50/40 p-4 space-y-3 dark:border-violet-800/50 dark:bg-violet-950/20">
            <div class="flex items-center justify-between">
                <p class="text-xs font-black text-violet-700 dark:text-violet-300">{{ __('messages.settings_delivery_new_payment') }}</p>
                <button type="button" @click="showPayForm = false" class="rounded-lg px-2 py-1 text-xs font-bold text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800">✕ {{ __('messages.settings_delivery_close') }}</button>
            </div>
            <form method="POST" action="{{ url('/store/' . $store->slug . '/admin/settings/payment-methods') }}" enctype="multipart/form-data" class="space-y-3"
                ...
                <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
                    <div>
                        <label class="{{ $labelClass }}">{{ __('messages.settings_delivery_method_name') }} <span class="text-rose-500">*</span></label>
                    ...
                    <div>
                        <label class="{{ $labelClass }}">{{ __('messages.settings_delivery_type') }}</label>
                    ...
                    <div>
                        <label class="{{ $labelClass }}">{{ __('messages.settings_delivery_icon_type') }}</label>
                        <select name="icon_type" x-model="payForm.icon_type" class="{{ $inputClass }}">
                            <option value="builtin">{{ __('messages.settings_delivery_icon_builtin') }}</option>
                            <option value="custom">{{ __('messages.settings_delivery_icon_custom') }}</option>
                            <option value="initials">{{ __('messages.settings_delivery_icon_initials') }}</option>
                        </select>
                    </div>
                    <div>
                        <label class="{{ $labelClass }}">{{ __('messages.settings_delivery_icon_key') }}</label>
                    ...
                    <div>
                        <label class="{{ $labelClass }}">{{ __('messages.settings_delivery_custom_icon') }}</label>
                    ...
                    <div>
                        <label class="{{ $labelClass }}">{{ __('messages.settings_delivery_qr_image') }}</label>
                    ...
                    <div>
                        <label class="{{ $labelClass }}">{{ __('messages.settings_delivery_account_name') }}</label>
                        <input type="text" name="account_name" x-model="payForm.account_name" maxlength="120" placeholder="e.g. U Thit Sar" class="{{ $inputClass }}" />
                    </div>
                    <div>
                        <label class="{{ $labelClass }}">{{ __('messages.settings_delivery_account_number') }}</label>
                    ...
                    <label class="inline-flex items-center gap-2 cursor-pointer">
                        ...
                        <span class="text-xs font-bold text-gray-700 dark:text-slate-200">{{ __('messages.settings_delivery_active') }}</span>
                    </label>
                    <label class="inline-flex items-center gap-2 cursor-pointer">
                        ...
                        <span class="text-xs font-bold text-gray-700 dark:text-slate-200">{{ __('messages.settings_delivery_show_details') }}</span>
                    </label>
                </div>
                <button type="submit"
            ...
            <div x-data="{ editing: false }" class="bg-white dark:bg-slate-800/60 rounded-xl px-3 py-2.5 sm:px-4 sm:py-3 transition-colors duration-200 border border-slate-200/80 dark:border-slate-700/60">
                    ...
                    <div class="flex items-center gap-2.5 sm:gap-3 min-w-0">
                        ...
                        <div class="min-w-0 flex flex-wrap items-center gap-2">
                            ...
                            <span class="rounded-full px-2 py-0.5 text-[9px] font-black {{ $pm->is_active ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-950/60 dark:text-emerald-300' : 'bg-slate-200 text-slate-600 dark:bg-slate-700 dark:text-slate-300' }}">
                                {{ $pm->is_active ? __('messages.settings_delivery_active_short') : __('messages.settings_delivery_inactive') }}
                            </span>
                            ...
                            @if ($pm->icon_path && \App\Support\StorefrontAsset::imageUrl($pm->icon_path))
                                <span class="inline-flex items-center gap-1 text-[9px] font-bold text-violet-600 dark:text-violet-400 bg-violet-50 dark:bg-violet-950/40 px-1.5 py-0.5 rounded border border-violet-200 dark:border-violet-800">
                                    <span>{{ __('messages.settings_delivery_custom_icon_badge') }}</span>
                                </span>
                            @endif
                        </div>
                    </div>
                    <div class="flex items-center gap-1.5 sm:gap-2 flex-wrap">
                        @if ($pm->hasQr())
                            <a href="{{ $pm->qrUrl() }}" target="_blank" rel="noopener noreferrer"
                                ...
                                title="{{ __('messages.settings_delivery_view_qr') }}">
                                <span>{{ __('messages.settings_delivery_view_qr') }}</span>
                            </a>
                        @endif
                        <button type="button" @click="editing = !editing"
                            ...
                            <span x-text="editing ? '✕ ' + @js(__('messages.settings_delivery_cancel')) : @js(__('messages.settings_delivery_edit'))"></span>
                        </button>
                        <form method="POST" action="{{ url('/store/' . $store->slug . '/admin/settings/payment-methods/' . $pm->id) }}" data-confirm="Delete {{ $pm->name }}?">
                            ...
                            <button type="submit" class="inline-flex min-h-8 items-center gap-1 rounded-lg bg-rose-600 px-2.5 py-1 text-[11px] font-black text-white hover:bg-rose-700 active:bg-rose-800 cursor-pointer shadow-xs transition">
                                <span>{{ __('messages.settings_delivery_delete') }}</span>
                            </button>
                        </form>
                    </div>
                </div>
                ...
                <div x-show="editing" x-transition:enter="transition ease-out duration-150" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" x-transition:leave="transition ease-in duration-100" x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-95" class="mt-3 pt-3 border-t border-slate-200/80 dark:border-slate-700/80">
                    <form method="POST" action="{{ url('/store/' . $store->slug . '/admin/settings/payment-methods/' . $pm->id) }}" enctype="multipart/form-data" class="space-y-4">
                        ...
                        <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
                            <div>
                                <label class="{{ $labelClass }}">{{ __('messages.settings_delivery_payment_name_edit') }}</label>
                                <input type="text" name="name" value="{{ $pm->name }}" required maxlength="120" class="{{ $inputClass }}" />
                            </div>
                            <div>
                                <label class="{{ $labelClass }}">{{ __('messages.settings_delivery_icon_type') }}</label>
                                <select name="icon_type" class="{{ $inputClass }}">
                                    <option value="builtin" {{ $pm->icon_type === 'builtin' ? 'selected' : '' }}>{{ __('messages.settings_delivery_icon_builtin') }}</option>
                                    <option value="custom" {{ $pm->icon_type === 'custom' ? 'selected' : '' }}>{{ __('messages.settings_delivery_icon_custom') }}</option>
                                    <option value="initials" {{ $pm->icon_type === 'initials' ? 'selected' : '' }}>{{ __('messages.settings_delivery_icon_initials') }}</option>
                                </select>
                            </div>
                            <div>
                                <label class="{{ $labelClass }}">{{ __('messages.settings_delivery_icon_key') }}</label>
                            ...
                            <div>
                                <label class="{{ $labelClass }}">{{ __('messages.settings_delivery_sort_order') }}</label>
                            ...
                            <div class="rounded-xl border border-slate-200 dark:border-slate-700/80 bg-slate-50/70 dark:bg-slate-900/40 p-3 space-y-2">
                                <label class="block text-xs font-black text-slate-800 dark:text-slate-200">📷 {{ __('messages.settings_delivery_custom_icon') }}</label>
                                @if ($pm->icon_path && \App\Support\StorefrontAsset::imageUrl($pm->icon_path))
                                    <div class="flex items-center gap-3">
                                        ...
                                        <label class="inline-flex items-center gap-1.5 text-xs font-bold text-rose-600 cursor-pointer">
                                            ...
                                            <span>{{ __('messages.settings_delivery_remove_icon') }}</span>
                                        </label>
                                    </div>
                                @endif
                                <input type="file" name="icon_image" accept="image/png,image/jpeg,image/webp"
                            ...
                            <div class="rounded-xl border border-slate-200 dark:border-slate-700/80 bg-slate-50/70 dark:bg-slate-900/40 p-3 space-y-2">
                                <label class="block text-xs font-black text-slate-800 dark:text-slate-200">📱 {{ __('messages.settings_delivery_qr_image') }}</label>
                                @if ($pm->hasQr())
                                    <div class="flex items-center gap-3">
                                        ...
                                        <label class="inline-flex items-center gap-1.5 text-xs font-bold text-rose-600 cursor-pointer">
                                            ...
                                            <span>{{ __('messages.settings_delivery_remove_qr') }}</span>
                                        </label>
                                    </div>
                                @endif
                                <input type="file" name="qr_image" accept="image/png,image/jpeg,image/webp"
                        ...
                        <div class="flex items-center gap-2 pt-2">
                            <button type="submit" class="inline-flex min-h-11 items-center rounded-xl bg-violet-600 px-5 py-2.5 text-xs font-black text-white hover:bg-violet-700 shadow-sm transition">
                                {{ __('messages.settings_delivery_save_changes') }}
                            </button>
                            <button type="button" @click="editing = false" class="inline-flex min-h-11 items-center rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-xs font-bold text-slate-700 hover:bg-slate-50 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-200">
                                {{ __('messages.settings_delivery_cancel') }}
                            </button>
                        </div>
                    </form>
                </div>
        ...
        @empty
            <div class="rounded-2xl border border-dashed border-slate-300 p-6 text-center dark:border-slate-700">
                <p class="text-sm font-bold text-slate-500 dark:text-slate-400">{{ __('messages.settings_delivery_none_payment') }}</p>
                <p class="mt-1 text-xs text-slate-400 dark:text-slate-500">{{ __('messages.settings_delivery_none_payment_hint') }}</p>
            </div>
        @endforelse
    ...
    <section class="space-y-3">
        ...
        <div x-show="showDelForm" x-cloak x-transition class="rounded-2xl border border-sky-200 bg-sky-50/40 p-4 space-y-3 dark:border-sky-800/50 dark:bg-sky-950/20">
            <div class="flex items-center justify-between">
                <p class="text-xs font-black text-sky-700 dark:text-sky-300">{{ __('messages.settings_delivery_new_delivery') }}</p>
                <button type="button" @click="showDelForm = false" class="rounded-lg px-2 py-1 text-xs font-bold text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800">✕ {{ __('messages.settings_delivery_close') }}</button>
            </div>
            <form method="POST" action="{{ url('/store/' . $store->slug . '/admin/settings/delivery-methods') }}" class="space-y-3">
                @csrf
                <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
                    <div>
                        <label class="{{ $labelClass }}">{{ __('messages.settings_delivery_name') }} <span class="text-rose-500">*</span></label>
                    ...
                    <div>
                        <label class="{{ $labelClass }}">{{ __('messages.settings_delivery_icon_emoji') }}</label>
                        <input type="text" name="icon" x-model="delForm.icon" maxlength="10" placeholder="🚚" class="{{ $inputClass }}" />
                    </div>
                    <div>
                        <label class="{{ $labelClass }}">{{ __('messages.settings_delivery_eta') }}</label>
                    ...
                    <div>
                        <label class="{{ $labelClass }}">{{ __('messages.settings_delivery_area') }}</label>
                    ...
                    <div>
                        <label class="{{ $labelClass }}">{{ __('messages.settings_delivery_fee_note') }}</label>
                    ...
                    <div class="sm:col-span-2">
                        <label class="{{ $labelClass }}">{{ __('messages.settings_delivery_desc') }}</label>
                    ...
                    <label class="inline-flex items-center gap-2 cursor-pointer">
                        ...
                        <span class="text-xs font-bold text-gray-700 dark:text-slate-200">{{ __('messages.settings_delivery_active_short') }}</span>
                    </label>
                </div>
                    ...
                    <div class="min-w-0 flex-1">
                        <div class="flex flex-wrap items-center gap-2">
                            ...
                            <span class="rounded-full px-2 py-0.5 text-[10px] font-black {{ $dm->is_active ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-950/60 dark:text-emerald-300' : 'bg-slate-200 text-slate-600 dark:bg-slate-700 dark:text-slate-300' }}">
                                {{ $dm->is_active ? __('messages.settings_delivery_active_short') : __('messages.settings_delivery_inactive') }}
                            </span>
        ...
        @empty
            <div class="rounded-2xl border border-dashed border-slate-300 p-6 text-center dark:border-slate-700">
                <p class="text-sm font-bold text-slate-500 dark:text-slate-400">{{ __('messages.settings_delivery_none_delivery') }}</p>
                <p class="mt-1 text-xs text-slate-400 dark:text-slate-500">{{ __('messages.settings_delivery_none_delivery_hint') }}</p>
            </div>
        @endforelse
    ...
    <form method="POST" action="{{ url('/store/' . $store->slug . '/admin/settings') }}" enctype="multipart/form-data"
        ...
        <div class="flex items-center justify-between gap-3 flex-wrap">
            ...
            <button type="submit" :disabled="submitting"
                ...
                <span x-show="submitting" x-cloak>{{ __('messages.settings_delivery_saving') }}</span>
        ...
        <div class="md:col-span-2">
            ...
            <div class="mt-2">
                ...
                <div class="mt-3 overflow-hidden rounded-xl border border-slate-200 dark:border-slate-700">
                    <p class="border-b border-slate-200 bg-slate-50 px-4 py-2 text-[10px] font-black uppercase tracking-widest text-slate-400 dark:border-slate-700 dark:bg-slate-800/60 dark:text-slate-500">{{ __('messages.settings_delivery_footer_preview') }}</p>
