            ...
            <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200/80 dark:border-slate-800 shadow-2xs hover:border-violet-300 dark:hover:border-violet-600/50 hover:shadow-sm transition flex flex-col justify-between group overflow-hidden">
                ...
                <div class="p-3 sm:p-3.5 space-y-2.5">
                    ...
                    <div class="flex items-center justify-between gap-1.5 text-xs">
                        ...
                        @if($session->scope === 'category')
                            <span class="shrink-0 inline-flex items-center gap-1 px-1.5 py-0.5 text-[10px] font-bold rounded-md bg-blue-50 text-blue-700 dark:bg-blue-950/60 dark:text-blue-300 border border-blue-200 dark:border-blue-800">
                                <span>🏷️</span>
                                <span>Category @if(!empty($session->category_ids))({{ count($session->category_ids) }})@endif</span>
                            </span>
                        @else
                            <span class="shrink-0 inline-flex items-center gap-1 px-1.5 py-0.5 text-[10px] font-bold rounded-md bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-300 border border-slate-200 dark:border-slate-700">
                                <span>📦</span>
                    ...
                    <div class="p-2.5 rounded-lg bg-slate-50/80 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 space-y-1.5">
                        <div class="flex items-center justify-between text-xs font-mono">
                            ...
                            <span class="font-black text-slate-900 dark:text-slate-100">
                                {{ number_format($session->counted_items) }} / {{ number_format($session->total_items) }}
                                <span class="text-[11px] text-violet-600 dark:text-violet-400 font-bold">({{ $progressPct }}%)</span>
                            </span>
                        </div>
                        <div class="w-full bg-slate-200 dark:bg-slate-700 rounded-full h-2 overflow-hidden">
                            <div class="h-2 rounded-full transition-all duration-300 {{ $progressPct === 100 ? 'bg-emerald-500' : 'bg-gradient-to-r from-violet-600 to-indigo-600' }}" style="width: {{ $progressPct }}%"></div>
                        </div>

                        {{-- Variance Status Row --}}
                        <div class="flex items-center justify-between text-xs pt-1 border-t border-slate-200/60 dark:border-slate-700/60">
                            <span class="text-[11px] text-slate-500 dark:text-slate-400 font-medium">{{ __('messages.stock_count_variance_items') }}:</span>
                            @if($session->variance_items > 0)
                                <span class="inline-flex items-center gap-1 px-1.5 py-0.2 rounded text-[11px] font-bold font-mono text-amber-700 dark:text-amber-300 bg-amber-50 dark:bg-amber-950/60 border border-amber-200 dark:border-amber-800">
                                    <span>⚠️</span>
                                    <span>{{ number_format($session->variance_items) }} items</span>
                                </span>
                            @else
                                <span class="text-[11px] font-mono font-bold text-emerald-600 dark:text-emerald-400 flex items-center gap-1">
                    ...
                    <div class="text-[11px] text-slate-500 dark:text-slate-400 flex items-center justify-between pt-1">
                        <span>ဖန်တီးသူ: <strong class="text-slate-700 dark:text-slate-300">{{ $session->createdBy?->name ?? 'System' }}</strong></span>
                        @if($session->branch && $session->warehouse)
                            <span class="text-[10px] text-slate-400">({{ $session->branch->name }})</span>
                        @endif
                    </div>
                    ...
                    @if($session->notes)
                        <div class="text-[10px] text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-800/40 p-1.5 rounded border border-slate-200/60 dark:border-slate-800 line-clamp-2">
                            <strong>မှတ်ချက်:</strong> {{ $session->notes }}
                        </div>
                    @endif
                </div>
                ...
                <div class="p-2.5 bg-slate-50/80 dark:bg-slate-800/40 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between gap-2">
                    ...
                    <a href="{{ route('store.admin.stock_count.show', ['store_slug' => $store->slug, 'stock_count' => $session->id]) }}"
                       class="px-3 py-1.5 rounded-lg text-xs font-bold transition flex items-center gap-1.5 shadow-2xs {{ $session->isInProgress() ? 'bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-500 hover:to-indigo-500 text-white shadow-md shadow-violet-900/20 active:scale-95' : 'bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 hover:bg-slate-200 dark:hover:bg-slate-700 active:scale-95' }}">
                        <span>{{ $session->isInProgress() ? __('messages.stock_count_continue_count') : __('messages.stock_count_view_audit') }}</span>
                        <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
                        </svg>
