...
<div class="max-w-4xl mx-auto space-y-5 sm:space-y-6">
    ...
    <div class="admin-page-header">
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <p class="admin-page-sub">{{ $store->name }} Â· {{ __('messages.repair_edit') }}</p>
            </div>
        </div>
    </div>
    ...
    <form method="POST" action="{{ route('store.admin.service_jobs.update', [...$storeRouteParams, 'job' => $job->id]) }}"
        ...
        <fieldset class="space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="contact_name" value="{{ old('contact_name', $job->contact_name) }}" maxlength="120"
                ...
                <div>
                    ...
                    <input type="text" name="contact_phone" value="{{ old('contact_phone', $job->contact_phone) }}" maxlength="40"
                           class="w-full border dark:border-slate-600 rounded-lg px-3 py-2.5 text-sm bg-white dark:bg-slate-900 text-gray-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-violet-500" />
                </div>
            </div>
        </fieldset>
        ...
        <fieldset class="space-y-4 pt-4 border-t dark:border-slate-700">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="text" name="device_type" value="{{ old('device_type', $job->device_type) }}" required maxlength="60"
                           class="w-full border dark:border-slate-600 rounded-lg px-3 py-2.5 text-sm bg-white dark:bg-slate-900 text-gray-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-violet-500" />
                </div>
                <div>
                    <label class="block text-xs font-semibold text-gray-600 dark:text-slate-300 mb-1">{{ __('messages.repair_model') }}</label>
                    <input type="text" name="model" value="{{ old('model', $job->model) }}" maxlength="120"
                           class="w-full border dark:border-slate-600 rounded-lg px-3 py-2.5 text-sm bg-white dark:bg-slate-900 text-gray-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-violet-500" />
                </div>
                <div>
                    <label class="block text-xs font-semibold text-gray-600 dark:text-slate-300 mb-1">IMEI / Serial</label>
                    <input type="text" name="imei_serial" value="{{ old('imei_serial', $job->imei_serial) }}" maxlength="60"
            ...
            <div>
                ...
                <textarea name="reported_problem" rows="3" required maxlength="1000"
                          class="w-full border dark:border-slate-600 rounded-lg px-3 py-2.5 text-sm bg-white dark:bg-slate-900 text-gray-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-violet-500">{{ old('reported_problem', $job->reported_problem) }}</textarea>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <textarea name="intake_condition" rows="2" maxlength="1000"
                              class="w-full border dark:border-slate-600 rounded-lg px-3 py-2.5 text-sm bg-white dark:bg-slate-900 text-gray-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-violet-500">{{ old('intake_condition', $job->intake_condition) }}</textarea>
                </div>
                <div>
                    ...
                    <textarea name="accessories" rows="2" maxlength="500"
                              class="w-full border dark:border-slate-600 rounded-lg px-3 py-2.5 text-sm bg-white dark:bg-slate-900 text-gray-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-violet-500">{{ old('accessories', $job->accessories) }}</textarea>
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="diagnosis" rows="3" maxlength="1000" placeholder="{{ __('messages.repair_diagnosis_placeholder') }}"
                          class="w-full border dark:border-slate-600 rounded-lg px-3 py-2.5 text-sm bg-white dark:bg-slate-900 text-gray-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-violet-500">{{ old('diagnosis', $job->diagnosis) }}</textarea>
            </div>
        </fieldset>

        {{-- â”€â”€ Parts & Services (Repairs Center parity) â”€â”€ --}}
        <fieldset class="space-y-4 pt-4 border-t dark:border-slate-700" x-data="{
            ...
            @if ($job->items->where('is_deducted', true)->isNotEmpty())
                <p class="text-xs text-emerald-600 dark:text-emerald-400 flex items-center gap-1.5">
                    <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                    {{ $job->items->where('is_deducted', true)->count() }} {{ __('messages.repair_deducted') }} â€” {{ __('messages.repair_items_total_hint') }}
                </p>
            @endif
        ...
        {{-- â”€â”€ Assignment, Charges & Warranty â”€â”€ --}}
        <fieldset class="space-y-4 pt-4 border-t dark:border-slate-700">
            <legend class="text-sm font-bold text-gray-900 dark:text-white mb-2 float-left w-full flex items-center gap-1.5">
                ...
                <div>
                    ...
                    <select name="technician_id" class="w-full border dark:border-slate-600 rounded-lg px-3 py-2.5 text-sm bg-white dark:bg-slate-900 text-gray-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-violet-500">
                        ...
                        @foreach ($technicians as $tech)
                            <option value="{{ $tech->id }}" @selected(old('technician_id', $job->technician_id) == $tech->id)>{{ $tech->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    <label class="block text-xs font-semibold text-gray-600 dark:text-slate-300 mb-1">{{ __('messages.repair_voucher_no') }}</label>
                    <input type="text" name="voucher_no" value="{{ old('voucher_no', $job->voucher_no) }}" maxlength="40"
                           class="w-full border dark:border-slate-600 rounded-lg px-3 py-2.5 text-sm bg-white dark:bg-slate-900 text-gray-900 dark:text-slate-100 font-mono focus:outline-none focus:ring-2 focus:ring-violet-500" />
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="date" name="estimated_completion"
                           value="{{ old('estimated_completion', $job->estimated_completion?->format('Y-m-d')) }}"
                           class="w-full border dark:border-slate-600 rounded-lg px-3 py-2.5 text-sm bg-white dark:bg-slate-900 text-gray-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-violet-500" />
                </div>
                <div>
                    <label class="block text-xs font-semibold text-gray-600 dark:text-slate-300 mb-1">{{ __('messages.repair_warranty_notes') }}</label>
                    <input type="text" name="warranty_notes" value="{{ old('warranty_notes', $job->warranty_notes) }}" maxlength="1000"
            ...
            <div>
                ...
                <textarea name="notes" rows="2" maxlength="1000"
                          class="w-full border dark:border-slate-600 rounded-lg px-3 py-2.5 text-sm bg-white dark:bg-slate-900 text-gray-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-violet-500">{{ old('notes', $job->notes) }}</textarea>
            </div>
        </fieldset>
