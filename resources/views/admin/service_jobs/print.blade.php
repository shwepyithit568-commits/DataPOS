...
<head>
    ...
    <title>{{ __('messages.repair_ticket_heading') }} {{ $job->job_number }} â€” {{ $store->name }}</title>
    <style>
        @if ($myanmarFontUrl)
        @font-face {
...
<body>
    <div class="ticket">
        <div class="ticket-head">
            ...
            <div class="store-meta">{{ $store->slug }} Â· {{ __('messages.sidebar_service_jobs') }}</div>
        </div>

        <hr class="rule">
        <div class="ticket-head">
            <div class="ticket-title">{{ __('messages.repair_ticket_heading') }}</div>
            <div class="job-no">{{ $job->job_number }}</div>
        </div>

        <div class="meta-row" style="margin-top:8px;">
            <span class="label">{{ __('messages.repair_received_at') }}</span>
            <span>{{ $job->created_at->format('M d, Y H:i') }}</span>
        </div>
        <div class="meta-row">
        ...
        @if ($job->estimated_completion)
            <div class="meta-row">
                ...
                <span>{{ $job->estimated_completion->format('M d, Y') }}</span>
            </div>
        @endif
        @if ($job->voucher_no)
            <div class="meta-row">
                <span class="label">{{ __('messages.repair_voucher_no') }}</span>
                <span>{{ $job->voucher_no }}</span>
            </div>
        @endif
        ...
        <div class="kv"><span class="label">{{ __('messages.repair_customer_label') }}</span>
            <span class="value">{{ $job->customer?->name ?? $job->contact_name ?? 'â€”' }}</span></div>
        @if ($job->contact_phone || $job->customer?->phone)
            <div class="kv"><span class="label">{{ __('messages.repair_contact_phone') }}</span>
                <span class="value">{{ $job->contact_phone ?? $job->customer->phone }}</span></div>
        @endif
        ...
        <div class="kv"><span class="label">{{ __('messages.repair_device_type') }}</span>
            <span class="value">{{ $job->device_type }}</span></div>
        @if ($job->model)
        ...
        <div class="kv" style="margin-top:5px;"><span class="label">{{ __('messages.repair_ticket_problem') }}</span>
            <span class="value">{{ $job->reported_problem }}</span></div>
            ...
            <table class="items">
                ...
                <tfoot>
                    <tr>
                        ...
                        <td class="num">{{ number_format($job->itemsTotal(), 0) }} MMK</td>
                    </tr>
                </tfoot>
            </table>
        @endif
        ...
        <div class="money-grid">
            <div><span class="label">{{ __('messages.repair_estimated_charge') }}</span>
                <div class="value">{{ number_format((float) $job->estimated_charge, 0) }} MMK</div></div>
            <div><span class="label">{{ __('messages.repair_final_charge') }}</span>
                <div class="value">{{ $job->final_charge !== null ? number_format((float) $job->final_charge, 0) . ' MMK' : 'â€”' }}</div></div>
            <div><span class="label">{{ __('messages.repair_ticket_paid') }}</span>
                <div class="value">{{ number_format($job->paidAmount(), 0) }} MMK</div></div>
            <div class="balance"><span class="label">{{ __('messages.repair_ticket_balance') }}</span>
                <div class="value">{{ number_format($job->outstanding(), 0) }} MMK</div></div>
        </div>

        @if ($job->warranty_notes)
            <hr class="rule">
            <div class="section-title">{{ __('messages.repair_warranty_notes') }}</div>
            <div style="font-size:12px;">{{ $job->warranty_notes }}</div>
        @endif

        <hr class="rule">
        @if ($job->tracking_token)
            <div class="footer-note" style="font-weight:700; color:#0f766e; margin-bottom: 4px;">
                🌐 Status တိုက်ရိုက်စစ်ဆေးရန်: {{ $job->trackingUrl($store->slug) }}
            </div>
        @endif
