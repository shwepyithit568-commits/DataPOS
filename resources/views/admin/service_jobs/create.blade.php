...
<div class="w-full space-y-2.5 sm:space-y-3 pb-8" x-data="repairTicketForm({
    ...
    <form method="POST" action="{{ route('store.admin.service_jobs.store', $storeRouteParams) }}" id="repairForm" class="space-y-4">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                ...
                <div>
                    <div class="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-2.5 mb-3">
                        <div class="flex items-center gap-2">
                            <span class="w-7 h-7 rounded-md bg-sky-50 dark:bg-sky-950/60 text-sky-600 dark:text-sky-400 grid place-items-center text-sm font-bold">
                                👤
                            </span>
                            <div>
                        ...
                        <button type="button" @click="openCustomerModal()"
                                class="px-2.5 py-1 rounded-md text-xs font-black bg-sky-600 hover:bg-sky-500 text-white transition flex items-center gap-1 shadow-2xs cursor-pointer">
                            <span>+</span>
                        ...
                        <div>
                            <div class="flex items-center justify-between mb-1">
                                ...
                                <template x-if="customerId">
                                    <span class="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-bold bg-emerald-50 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800">
                            ...
                            <div class="relative">
                                ...
                                <button type="button" x-show="contactName" @click="contactName = ''; customerId = '';"
                                        class="absolute right-2.5 top-1/2 -translate-y-1/2 text-xs font-bold text-slate-400 hover:text-slate-600">✕</button>

                                {{-- Autocomplete Dropdown List --}}
                                <div x-show="showCustomerDropdown && filteredCustomers.length > 0" @click.outside="showCustomerDropdown = false" x-cloak
                                     class="absolute z-30 inset-x-0 top-full mt-1 max-h-56 overflow-y-auto rounded-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 shadow-xl divide-y divide-slate-100 dark:divide-slate-800">
                                    <template x-for="c in filteredCustomers" :key="c.id">
                                        <button type="button" @click="selectCustomer(c)"
                                                class="w-full text-left px-3 py-2 text-xs hover:bg-sky-50 dark:hover:bg-slate-800 flex items-center justify-between transition group">
                                            <div>
                                                ...
                                                <p class="text-[10px] text-slate-400 truncate max-w-xs" x-text="c.address || 'No Address'"></p>
                                            </div>
                                            <span class="text-[10px] px-2 py-0.5 rounded bg-slate-100 dark:bg-slate-800 font-black text-slate-600 dark:text-slate-300 group-hover:bg-sky-600 group-hover:text-white transition">
                                                {{ __('messages.select') }}
                                            </span>
                                        </button>
                                    </template>
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="tel" name="contact_phone" x-model="contactPhone" maxlength="40"
                                           ...
                                           class="w-full pl-8 pr-3 py-2 rounded-lg border bg-slate-50 dark:bg-slate-800/60 text-xs font-mono font-semibold focus:bg-white dark:focus:bg-slate-900 focus:ring-2 focus:ring-sky-500 outline-none transition"
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <textarea name="shipping_address" x-model="shippingAddress" rows="1" maxlength="500"
                                              ...
                                              class="w-full pl-8 pr-3 py-1.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/60 text-xs font-medium focus:bg-white dark:focus:bg-slate-900 focus:ring-2 focus:ring-sky-500 outline-none transition resize-none"></textarea>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            {{-- 2. Status & Payment Card --}}
            <div class="rounded-lg bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 p-4 sm:p-5 shadow-2xs flex flex-col justify-between space-y-3">
                <div>
                    <div class="flex items-center gap-2 border-b border-slate-100 dark:border-slate-800 pb-2.5 mb-3">
                        <span class="w-7 h-7 rounded-md bg-teal-50 dark:bg-teal-950/60 text-teal-600 dark:text-teal-400 grid place-items-center text-sm font-bold">
                            ✔️
                        </span>
                        <div>
                            <h2 class="text-xs sm:text-sm font-black text-slate-900 dark:text-white">
                            ...
                            <div class="min-w-0">
                                ...
                                <div class="flex items-center gap-1.5 min-w-0">
                                    <select name="status" x-model="selectedStatus" x-ref="statusSelect"
                                            class="flex-1 min-w-0 w-full px-2.5 py-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/60 text-xs font-bold text-slate-800 dark:text-slate-200 focus:bg-white dark:focus:bg-slate-900 focus:ring-2 focus:ring-teal-500 outline-none truncate">
                                        @foreach ($statuses as $st)
                                            <option value="{{ $st->code ?: $st->name }}">{{ $st->name }}</option>
                                        @endforeach
                                    </select>
                                    <button type="button" @click="openQuickAdd('status', '{{ __('messages.repair_statuses') }}')"
                                            class="w-8 h-8 shrink-0 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-sm grid place-items-center shadow-2xs transition cursor-pointer"
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-0.5">
                            ...
                            <div class="min-w-0">
                                ...
                                <select name="payment_method"
                                        class="w-full px-2.5 py-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/60 text-xs font-bold text-slate-800 dark:text-slate-200 focus:bg-white dark:focus:bg-slate-900 focus:ring-2 focus:ring-teal-500 outline-none truncate">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-0.5">
                            <div class="min-w-0">
                                ...
                                <input type="number" name="estimated_charge" x-model="estimatedCharge" min="0" step="100"
                                       placeholder="0"
                                       class="w-full px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/60 text-xs font-mono font-bold focus:bg-white dark:focus:bg-slate-900 focus:ring-2 focus:ring-teal-500 outline-none">
                            </div>
                            <div class="min-w-0">
                                <label class="block text-xs font-bold text-slate-600 dark:text-slate-400 mb-1">
                                ...
                                <input type="text" name="voucher_no" maxlength="40"
                                       ...
                                       class="w-full px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/60 text-xs font-mono font-semibold focus:bg-white dark:focus:bg-slate-900 focus:ring-2 focus:ring-teal-500 outline-none">
                            </div>
                        </div>
                    </div>
                </div>
        ...
        {{-- Row 2: Device & Issue Section --}}
        <div class="rounded-lg bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 p-4 sm:p-5 shadow-2xs space-y-4">
            <div class="flex items-center gap-2 border-b border-slate-100 dark:border-slate-800 pb-2.5">
                <span class="w-7 h-7 rounded-md bg-amber-50 dark:bg-amber-950/60 text-amber-600 dark:text-amber-400 grid place-items-center text-sm font-bold">
                    📱
                </span>
                <h2 class="text-xs sm:text-sm font-black text-slate-900 dark:text-white">
            ...
            <div class="space-y-3">
                    ...
                    <div class="min-w-0">
                        ...
                        <div class="flex items-center gap-1.5 min-w-0">
                            <select name="brand" x-model="selectedBrand" x-ref="brandSelect"
                                    class="flex-1 min-w-0 w-full px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/60 text-xs font-bold text-slate-800 dark:text-slate-200 focus:bg-white dark:focus:bg-slate-900 focus:ring-2 focus:ring-amber-500 outline-none truncate">
                    ...
                    <div class="min-w-0">
                        ...
                        <div class="flex items-center gap-1.5 min-w-0">
                            <input type="text" name="model" x-model="modelName" x-ref="modelInput" list="modelsDatalist" required maxlength="120"
                                   placeholder="e.g. iPhone 13 Pro Max, Dell Latitude 5420, Dahua IP Camera"
                                   class="flex-1 min-w-0 w-full px-3 py-2 rounded-lg border bg-slate-50 dark:bg-slate-800/60 text-xs font-semibold focus:bg-white dark:focus:bg-slate-900 focus:ring-2 focus:ring-amber-500 outline-none transition truncate"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                    ...
                    <div class="min-w-0">
                        ...
                        <input type="text" name="imei_serial" maxlength="60"
                               ...
                               class="w-full px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/60 text-xs font-mono font-semibold focus:bg-white dark:focus:bg-slate-900 focus:ring-2 focus:ring-amber-500 outline-none transition">
                    </div>
                </div>
                ...
                <div class="min-w-0">
                    ...
                    <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 min-w-0">
                        <input type="text" name="reported_problem" x-model="reportedProblem" required maxlength="1000"
                               placeholder="e.g. Screen Broken, No Power, Water Damage, Windows Blue Screen, Network disconnect..."
                               class="flex-1 min-w-0 w-full px-3 py-2 rounded-lg border bg-slate-50 dark:bg-slate-800/60 text-xs font-semibold focus:bg-white dark:focus:bg-slate-900 focus:ring-2 focus:ring-amber-500 outline-none transition"
                        ...
                        <div class="flex items-center gap-1.5 shrink-0">
                            <select x-ref="defectSelect" @change="if($event.target.value) reportedProblem = $event.target.value"
                                    class="w-40 px-2.5 py-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/60 text-xs font-bold text-slate-700 dark:text-slate-300 truncate">
                ...
                <div class="min-w-0">
                    ...
                    <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 min-w-0">
                        <input type="text" name="accessories" x-model="accessoriesText" maxlength="500"
                               placeholder="e.g. Charger, Power Adapter, Bag, Mouse, SIM Tray..."
                               class="flex-1 min-w-0 w-full px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/60 text-xs font-semibold focus:bg-white dark:focus:bg-slate-900 focus:ring-2 focus:ring-amber-500 outline-none transition">
                        <div class="flex items-center gap-1.5 shrink-0">
                ...
                <div class="rounded-lg bg-slate-50/80 dark:bg-slate-800/40 border border-slate-200/80 dark:border-slate-700/60 p-3.5 space-y-3">
                    ...
                    {{-- Pattern Lock Canvas --}}
                    <div x-show="usePatternLock" x-cloak class="p-3 rounded-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 space-y-2.5">
                        <div class="flex items-center justify-between">
                        ...
                        <div class="flex flex-col sm:flex-row items-center gap-4 justify-center py-1">
                            <div class="grid grid-cols-3 gap-3 p-2.5 bg-slate-100 dark:bg-slate-800 rounded-xl">
                                <template x-for="dot in [1,2,3,4,5,6,7,8,9]" :key="dot">
                                    <button type="button" @click="pressDot(dot)"
                                            :class="patternSequence.includes(dot) ? 'bg-violet-600 text-white scale-105 shadow-md shadow-violet-500/40 ring-2 ring-violet-300 dark:ring-violet-900' : 'bg-white dark:bg-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-200'"
                                            class="w-9 h-9 rounded-full font-black text-xs grid place-items-center transition duration-150"
                                            x-text="dot"></button>
                                </template>
                            </div>

                            <div class="space-y-1 text-center sm:text-left">
                                <p class="text-[11px] text-slate-500 dark:text-slate-400">{{ __('messages.repair_pattern_sequence') }}:</p>
                                <div class="px-3 py-1.5 rounded-lg bg-slate-100 dark:bg-slate-800 text-xs font-mono font-bold text-violet-600 dark:text-violet-400 min-w-[140px]"
                    ...
                    <div>
                        ...
                        <input type="text" name="device_password" maxlength="120"
                               ...
                               class="w-full px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-xs font-mono font-semibold focus:ring-2 focus:ring-amber-500 outline-none transition">
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="rounded-lg bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 p-4 sm:p-5 shadow-2xs space-y-3">
            <div class="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-2.5">
                <div class="flex items-center gap-2">
                    <span class="w-7 h-7 rounded-md bg-emerald-50 dark:bg-emerald-950/60 text-emerald-600 dark:text-emerald-400 grid place-items-center text-sm font-bold">
                        🔧
                    </span>
                    <div>
                        <h2 class="text-xs sm:text-sm font-black text-slate-900 dark:text-white">
                        ...
                        <p class="text-[10px] text-slate-400">{{ __('messages.repair_items_hint') }}</p>
                    </div>
                </div>

                <button type="button" @click="addItem()"
                        class="px-2.5 py-1.5 rounded-lg text-xs font-bold bg-emerald-600 hover:bg-emerald-500 text-white transition flex items-center gap-1 shadow-2xs cursor-pointer">
            ...
            <template x-if="items.length > 0">
                <div class="space-y-2">
                    <template x-for="(item, i) in items" :key="i">
                        <div class="grid grid-cols-1 sm:grid-cols-12 gap-2 items-center p-2.5 rounded-lg bg-slate-50 dark:bg-slate-800/60 border border-slate-200/80 dark:border-slate-700">
                            {{-- Type --}}
                            <div class="sm:col-span-2">
                                <select :name="'items[' + i + '][item_type]'" x-model="item.item_type"
                                        class="w-full px-2 py-1.5 rounded-md border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-xs font-bold text-slate-700 dark:text-slate-200 outline-none">
                            ...
                            <div class="sm:col-span-4">
                                <input type="text" :name="'items[' + i + '][name]'" x-model="item.name" maxlength="120"
                                       ...
                                       class="w-full px-2.5 py-1.5 rounded-md border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-xs font-semibold outline-none focus:ring-2 focus:ring-emerald-500">
                            </div>
                            ...
                            <div class="sm:col-span-3" x-show="item.item_type === 'part'">
                                <select :name="'items[' + i + '][product_id]'" x-model="item.product_id" @change="onProductChange(i)"
                                        class="w-full px-2 py-1.5 rounded-md border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-xs font-semibold outline-none">
                            ...
                            <div class="sm:col-span-1">
                                <input type="number" :name="'items[' + i + '][quantity]'" x-model="item.quantity" min="1" step="1"
                                       class="w-full px-2 py-1.5 text-center rounded-md border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-xs font-mono font-bold outline-none">
                            </div>

                            {{-- Price --}}
                            <div class="sm:col-span-1">
                                <input type="number" :name="'items[' + i + '][unit_price]'" x-model="item.unit_price" min="0" step="100"
                                       class="w-full px-2 py-1.5 text-right rounded-md border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-xs font-mono font-bold outline-none">
                            </div>
                            ...
                            <div class="sm:col-span-1 text-right flex items-center justify-end gap-1">
                                ...
                                <button type="button" @click="removeItem(i)"
                                        class="w-7 h-7 rounded-md bg-rose-50 dark:bg-rose-950/60 text-rose-600 hover:bg-rose-100 transition grid place-items-center text-xs font-bold cursor-pointer">
                                    ✕
                                </button>
                            </div>
                        </div>
            ...
            <template x-if="items.length === 0">
                <div class="p-6 text-center text-slate-400 border border-dashed border-slate-200 dark:border-slate-800 rounded-lg">
    ...
    <div x-show="customerModalOpen" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs overflow-y-auto"
         @keydown.escape.window="customerModalOpen = false" @click.self="customerModalOpen = false">
        <div class="relative w-full max-w-md rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-5 shadow-2xl space-y-3.5 text-slate-900 dark:text-slate-100 animate-in fade-in zoom-in-95 duration-150">
            <div class="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-2.5">
                <div class="flex items-center gap-2">
                    <span class="w-7 h-7 rounded-md bg-sky-50 dark:bg-sky-950/60 text-sky-600 dark:text-sky-400 grid place-items-center text-sm font-bold">👤</span>
                    <h3 class="text-sm font-black text-slate-900 dark:text-white">{{ __('messages.create_new_customer') }}</h3>
                </div>
                <button type="button" @click="customerModalOpen = false" class="text-slate-400 hover:text-slate-600 text-sm font-bold">✕</button>
            </div>
            ...
            <div class="space-y-3 text-xs">
                <div>
                    ...
                    <input type="text" x-model="newCustomer.name" x-ref="newCustomerName"
                           ...
                           class="w-full px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-slate-100 font-semibold focus:outline-none focus:ring-2 focus:ring-sky-500">
                </div>
                ...
                <div>
                    ...
                    <input type="tel" x-model="newCustomer.phone"
                           placeholder="09 123 456 789"
                           class="w-full px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-slate-100 font-mono font-semibold focus:outline-none focus:ring-2 focus:ring-sky-500">
                </div>
                ...
                <div>
                    ...
                    <select x-model="newCustomer.type"
                            class="w-full px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-slate-100 font-bold focus:outline-none focus:ring-2 focus:ring-sky-500">
                ...
                <div>
                    ...
                    <textarea x-model="newCustomer.address" rows="2" maxlength="500"
                              placeholder="{{ __('messages.address') }}..."
                              class="w-full px-3 py-1.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-slate-100 font-medium focus:outline-none focus:ring-2 focus:ring-sky-500 resize-none"></textarea>
                </div>
                ...
                <div class="flex gap-2 pt-2 border-t border-slate-100 dark:border-slate-800">
                    <button type="button" @click="customerModalOpen = false"
                            class="flex-1 py-2 rounded-lg text-xs font-bold bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-200 transition">
                        {{ __('messages.cancel') }}
                    </button>
                    <button type="button" @click="submitNewCustomer()" :disabled="customerModalBusy || !newCustomer.name.trim() || !newCustomer.phone.trim()"
                            class="flex-1 py-2 rounded-lg text-xs font-black bg-sky-600 hover:bg-sky-500 text-white shadow-2xs disabled:opacity-50 transition flex items-center justify-center gap-1.5 cursor-pointer">
    ...
    {{-- Universal Quick-Add Modal for Master Data --}}
    <div x-show="quickAddModalOpen" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs overflow-y-auto"
         @keydown.escape.window="quickAddModalOpen = false" @click.self="quickAddModalOpen = false">
        <div class="relative w-full max-w-sm rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-5 shadow-2xl space-y-3.5 text-slate-900 dark:text-slate-100 animate-in fade-in zoom-in-95 duration-150">
            <div class="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-2.5">
                ...
                <button type="button" @click="quickAddModalOpen = false" class="text-slate-400 hover:text-slate-600 text-sm font-bold">✕</button>
            </div>
            ...
            <div class="space-y-3 text-xs">
                <div>
                    ...
                    <input type="text" x-model="quickAddName" x-ref="quickAddInput" @keydown.enter="submitQuickAdd()"
                           class="w-full px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-slate-100 font-semibold focus:outline-none focus:ring-2 focus:ring-emerald-500"
                ...
                <div class="flex gap-2 pt-2 border-t border-slate-100 dark:border-slate-800">
                    ...
                    <button type="button" @click="submitQuickAdd()" :disabled="quickAddBusy || !quickAddName.trim()"
                            class="flex-1 py-2 rounded-lg text-xs font-black bg-emerald-600 hover:bg-emerald-500 text-white shadow-2xs disabled:opacity-50 transition cursor-pointer">
...
function repairTicketForm(config) {
    return {
        ...
        async submitNewTechnician() {
            ...
            try {
                const res = await fetch('{{ route('store.admin.service_jobs.quick_add_technician', $storeRouteParams) }}', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/x-www-form-urlencoded',
                        'X-CSRF-TOKEN': this.csrf,
