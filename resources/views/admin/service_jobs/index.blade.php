            ...
            <div data-href="{{ $detailUrls[$job->id] }}" onclick="window.location.href = this.dataset.href"
                ...
                <div>
                    ...
                    <div class="relative h-28 bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-800/60 dark:to-slate-900 flex items-center justify-center border-b border-slate-100 dark:border-slate-800">
                        ...
                        @elseif ($icon === 'printer')
                            <svg class="w-12 h-12 text-slate-300 dark:text-slate-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4H7v4a2 2 0 002 2z"/></svg>
                        @else
                            <svg class="w-12 h-12 text-slate-300 dark:text-slate-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 18h.01M8 21h8a2 2 0 002-2V5a2 2 0 00-2-2H8a2 2 0 00-2 2v14a2 2 0 002 2z"/></svg>
                        @endif
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                    @forelse ($jobs as $job)
                        <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/40 transition">
                            <td class="p-2.5">
                                <a href="{{ route('store.admin.service_jobs.show', [...$storeRouteParams, 'job' => $job->id]) }}" class="font-mono font-bold text-slate-900 dark:text-slate-100 hover:text-teal-600 dark:hover:text-teal-400">{{ $job->job_number }}</a>
                            </td>
                            <td class="p-2.5">
                                ...
                                <div class="flex items-center justify-end gap-1">
                                    @if (! $job->isTerminal())
                                        <button type="button" @click="openQuickStatus({{ $job->id }}, '{{ $job->job_number }}', '{{ $job->status }}')"
                                            class="px-2 py-1 text-[10px] font-bold text-teal-700 dark:text-teal-300 bg-teal-50 dark:bg-teal-950/60 border border-teal-200 dark:border-teal-800 rounded-md hover:bg-teal-100 dark:hover:bg-teal-900/50 transition cursor-pointer">
                                            Status
                                        </button>
            ...
            <div class="flex items-start justify-between gap-3 border-b border-slate-100 dark:border-slate-800 pb-2.5">
                <div>
                    ...
                    <p class="text-[11px] text-slate-400 mt-0.5">
                        <span class="font-mono font-bold" x-text="jobNumber"></span> · Current: <span x-text="currentStatus" class="font-bold text-teal-600 dark:text-teal-400"></span>
                    </p>
                </div>
            ...
            <form method="POST" :action="statusUrl" class="space-y-3 text-xs">
                ...
                <div>
                    ...
                    <input type="text" name="note" x-model="note" maxlength="500"
                           ...
                           class="w-full border border-slate-200 dark:border-slate-700 rounded-lg px-2.5 py-1.5 text-xs bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-teal-500" />
                </div>
                ...
                <div class="flex items-center justify-end gap-2 pt-2.5 border-t border-slate-100 dark:border-slate-800">
                    ...
                    <button type="submit" :disabled="!selected"
                        class="px-4 py-2 text-xs font-bold text-white bg-teal-600 hover:bg-teal-700 rounded-lg shadow-2xs transition cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed active:scale-95">
