    ...
    @unless($embedded ?? false)
        ...
        <div class="p-2.5 sm:p-3 bg-white dark:bg-slate-900 rounded-lg border border-slate-200/80 dark:border-slate-800 shadow-2xs flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2.5">
            ...
            <button type="button" @click="$dispatch('open-brand-create')"
                    class="sf-btn-3d-primary px-3.5 py-1.5 rounded-lg text-xs font-black transition inline-flex items-center gap-1.5 shrink-0 cursor-pointer">
                                ...
                                <tr id="brand-row-{{ $brand->id }}"
                                    ...
                                    <td class="py-2 px-3">
                                        ...
                                        @if ($highlightBrand && (int) $highlightBrand === (int) $brand->id)
                                            <span class="inline-block mt-1 px-1.5 py-0.5 rounded-full bg-violet-100 dark:bg-violet-950/60 text-violet-700 dark:text-violet-300 text-[9px] font-black uppercase">{{ __('messages.condition_new') }}</span>
                                        @endif
                                    </td>
                    ...
                    <div id="brand-card-{{ $brand->id }}" data-brand-row="{{ $brand->id }}"
                        ...
                        <div class="p-3 space-y-2">
                            ...
                            <div class="mt-1">
                                ...
                                @if ($highlightBrand && (int) $highlightBrand === (int) $brand->id)
                                    <span class="ml-1 inline-flex items-center px-1.5 py-0.5 rounded bg-violet-100 dark:bg-violet-950/60 text-violet-700 dark:text-violet-300 text-[10px] font-bold">{{ __('messages.condition_new') }}</span>
                                @endif
                            </div>
                        </div>
                        ...
                        <div class="p-2.5 bg-slate-50/80 dark:bg-slate-800/40 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between gap-1.5 mt-2">
                            ...
                            @if ($brand->products_count > 0)
                                <span class="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-gray-100 dark:bg-slate-800 text-slate-400 dark:text-slate-500 text-xs font-semibold" title="{{ __('messages.brand_delete_blocked', ['count' => $brand->products_count]) }}">
                                    <span>🔒</span>
                                    <span>{{ __('messages.condition_used') }}</span>
                                </span>
                            @else
                                <button type="button" data-id="{{ $brand->id }}" data-name="{{ $brand->name }}"
                                    @click="openConfirm($el)"
                    ...
                    <form method="POST"
                        ...
                        <div class="flex items-center justify-end gap-2 pt-3 border-t border-slate-100 dark:border-slate-800">
                            <button type="button" @click="closeModal()"
                                class="sf-btn-3d px-4 py-2 rounded-lg text-xs font-bold transition cursor-pointer">
                                {{ __('messages.cancel') }}
                            </button>
                            <button type="submit" :disabled="saving"
                                class="sf-btn-3d-primary px-5 py-2 rounded-lg text-white text-xs font-black transition disabled:opacity-60 disabled:cursor-not-allowed cursor-pointer">
                    ...
                    <form x-ref="deleteForm" method="POST"
                        ...
                        <div class="flex items-center justify-center gap-2 pt-2 border-t border-slate-100 dark:border-slate-800">
                            ...
                            <button type="submit" @click="submitDelete()" :disabled="deleting"
                                class="sf-btn-3d-danger px-5 py-2 rounded-lg text-white text-xs font-black transition disabled:opacity-60 disabled:cursor-not-allowed cursor-pointer">
