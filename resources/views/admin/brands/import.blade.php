...
<div class="space-y-6 max-w-5xl">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <h1 class="text-2xl font-bold text-gray-900 dark:text-slate-100 font-outfit">Import Brands</h1>
        <div class="w-full sm:w-auto flex flex-nowrap items-center gap-2 overflow-x-auto pb-1 -mx-1 px-1 sm:flex-wrap sm:overflow-visible sm:mx-0 sm:px-0 sm:pb-0">
            <a href="{{ route('store.admin.brands.import.template', ['store_slug' => $store->slug]) }}" class="shrink-0 px-3 py-2 bg-green-600 text-white rounded-md text-xs font-semibold hover:bg-green-700 whitespace-nowrap">Download Brand Template</a>
            <a href="{{ route('store.admin.brands.index', ['store_slug' => $store->slug]) }}" class="shrink-0 text-xs text-violet-600 dark:text-violet-400 font-semibold hover:underline whitespace-nowrap">&larr; Back to Brands</a>
        </div>
    </div>
    ...
    @if (session('import_preview'))
        ...
        <div class="bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 rounded-lg p-4 space-y-4">
            ...
            <form method="POST" action="{{ route('store.admin.brands.import.confirm', ['store_slug' => $store->slug]) }}" class="flex flex-nowrap items-end gap-3 overflow-x-auto pb-1 -mx-1 px-1 sm:overflow-visible sm:mx-0 sm:px-0 sm:pb-0">
                ...
                <div class="shrink-0">
                    <label class="block text-xs font-semibold text-gray-700 dark:text-slate-300 mb-1">Duplicate Brand Handling</label>
                    <select name="duplicate_strategy" class="border dark:border-slate-600 rounded px-3 py-2 text-sm bg-white dark:bg-slate-900">
                        <option value="skip" {{ $preview['duplicate_strategy'] === 'skip' ? 'selected' : '' }}>Skip duplicate brands</option>
                        <option value="update" {{ $preview['duplicate_strategy'] === 'update' ? 'selected' : '' }}>Update existing brands</option>
                    </select>
                </div>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl p-6 shadow-sm space-y-5 transition-colors duration-200">
        <form action="{{ route('store.admin.brands.import', ['store_slug' => $store->slug]) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            ...
            <div>
                <label class="block text-sm font-semibold text-gray-700 dark:text-slate-300 mb-1">Duplicate Brand Handling</label>
                <select name="duplicate_strategy" class="border dark:border-slate-600 rounded px-3 py-2 text-sm bg-white dark:bg-slate-900">
                    <option value="skip">Skip duplicate brands (default)</option>
                    <option value="update">Update existing brands</option>
                </select>
            </div>
        ...
        <div class="border-t dark:border-slate-700 pt-4">
            ...
            <p class="text-xs text-gray-400 dark:text-slate-500 mt-2">
                <code>name</code> is required. <code>slug</code> is optional — leave blank to auto-generate from
                the name (lowercase letters, numbers and dashes only). Brands are matched by slug first, then by
                name (case-insensitive); matching brands are skipped or updated depending on the strategy.
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl p-6 shadow-sm space-y-3">
        <h2 class="text-lg font-bold text-gray-900 dark:text-slate-100">Recent Brand Imports</h2>
