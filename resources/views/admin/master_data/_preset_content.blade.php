                ...
                <thead class="sticky top-0 z-20 bg-slate-100 dark:bg-slate-800/95 backdrop-blur-xs border-b-2 border-slate-300 dark:border-slate-600 shadow-2xs select-none">
                    <tr class="text-[11px] font-black text-slate-700 dark:text-slate-200 uppercase tracking-wider divide-x divide-slate-300 dark:divide-slate-700">
                        @if ($presetType !== 'warranty' && $presetType !== 'return_policy')
                            <th class="py-2.5 px-3 min-w-[120px]">Short Code</th>
                        @endif
                        <th class="py-2.5 px-3 min-w-[200px]">Name / Details</th>
                        @if ($presetType === 'color')
                            <th class="py-2.5 px-3 min-w-[140px]">Color Preview</th>
                        @endif
                        @if ($presetType === 'warranty' || $presetType === 'return_policy' || $presetType === 'shelf_location')
                            <th class="py-2.5 px-3 min-w-[220px]">Description / Policy Terms</th>
                        @endif
                        <th class="py-2.5 px-3 text-center min-w-[100px]">Status</th>
                        <th class="py-2.5 px-3 text-right w-28">Actions</th>
                    </tr>
                </thead>
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="p-8 text-center text-slate-400">
                                ...
                                <p class="text-sm font-bold text-slate-700 dark:text-slate-300">No items found in {{ $title }}.</p>
                                <p class="text-xs text-slate-400 mt-1">Click "+ Add Item" above to create one.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
        ...
        <div class="min-h-full flex items-center justify-center p-4">
            <div class="relative w-full max-w-md bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-2xl p-5 space-y-4" @click.stop>
                <div class="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
                    <h3 class="text-sm sm:text-base font-black text-slate-900 dark:text-slate-100" x-text="modalMode === 'create' ? '➕ Add ' + '{{ $title }}' : '✏️ Edit ' + '{{ $title }}'"></h3>
        ...
        <div class="min-h-full flex items-center justify-center p-4">
            <div class="relative w-full max-w-sm bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-2xl p-5 space-y-4" @click.stop>
                <div class="text-center space-y-2">
                    ...
                    <h4 class="text-sm font-black text-slate-900 dark:text-slate-100">Delete Item?</h4>
                    <p class="text-xs text-slate-500 dark:text-slate-400">
                        Are you sure you want to delete <strong class="text-slate-900 dark:text-slate-100" x-text="deleteTarget?.name"></strong>?
                    </p>
                </div>
