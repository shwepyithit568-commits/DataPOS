...
@php
    /** @var array<int, array{filename: string, format: string, size: int, driver: string, created_at: \Illuminate\Support\Carbon}> $files */
    /** @var array{filename: string, format: string, size: int, driver: string, created_at: \Illuminate\Support\Carbon} $file */
...
<div class="w-full space-y-0.5 pb-6"
    ...
    <div class="px-2 py-1.5 bg-white dark:bg-slate-900 rounded border border-slate-200/90 dark:border-slate-800 shadow-2xs flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1.5 select-none transition">
        ...
        <div class="flex flex-wrap items-center gap-1 sm:gap-1.5 shrink-0 self-end sm:self-auto">
            ...
            <a href="{{ route('store.admin.database.index', $storeRouteParams) }}"
               class="sf-btn-3d h-7 px-2.5 rounded-md text-xs font-semibold inline-flex items-center gap-1">
                <span>🗄️</span>
    ...
    {{-- 2. 4 Key Stat KPI Cards (Standard v4.1 Centered Row-based) --}}
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-0.5 sm:gap-1 select-none">
        ...
        <div class="rounded border p-1.5 sm:p-2 shadow-2xs flex items-center justify-center gap-2.5 sm:gap-3 transition bg-white dark:bg-slate-900 border-slate-200/90 dark:border-slate-800"
             title="{{ __('messages.backup_total_backups') }}">
            ...
            <div class="min-w-0 text-left">
                <div class="text-[10px] sm:text-[11px] font-bold truncate text-slate-500 dark:text-slate-400">
                    {{ __('messages.backup_total_backups') }}
                </div>
        ...
        {{-- Total Size --}}
        <div class="rounded border p-1.5 sm:p-2 shadow-2xs flex items-center justify-center gap-2.5 sm:gap-3 transition bg-white dark:bg-slate-900 border-slate-200/90 dark:border-slate-800"
             title="{{ __('messages.backup_total_size') }}">
            ...
            <div class="min-w-0 text-left">
                ...
                <div class="text-xs sm:text-sm font-black text-indigo-600 dark:text-indigo-400 font-mono tracking-tight flex items-center gap-1">
                    <span>{{ $totalSize > 0 ? round($totalSize / 1024, 1) . ' KB' : '0 KB' }}</span>
                </div>
            </div>
        </div>
        ...
        {{-- Last Backup --}}
        <div class="rounded border p-1.5 sm:p-2 shadow-2xs flex items-center justify-center gap-2.5 sm:gap-3 transition bg-white dark:bg-slate-900 border-slate-200/90 dark:border-slate-800"
             title="{{ __('messages.backup_last_backup') }}">
            ...
            <div class="min-w-0 text-left">
                <div class="text-[10px] sm:text-[11px] font-bold truncate text-slate-500 dark:text-slate-400">
                    {{ __('messages.backup_last_backup') }}
                </div>
                <div class="text-xs sm:text-sm font-black text-emerald-600 dark:text-emerald-400 font-mono tracking-tight flex items-center gap-1">
                    <span>{{ $lastBackup ? $lastBackup->format('d M, h:i A') : __('messages.backup_never') }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="rounded border p-1.5 sm:p-2 shadow-2xs flex items-center justify-center gap-2.5 sm:gap-3 transition bg-white dark:bg-slate-900 border-slate-200/90 dark:border-slate-800"
            ...
            <div class="min-w-0 text-left">
                <div class="text-[10px] sm:text-[11px] font-bold truncate text-slate-500 dark:text-slate-400">
                    {{ __('messages.backup_type') }}
                </div>
                <div class="text-xs sm:text-sm font-black text-sky-600 dark:text-sky-400 font-mono tracking-tight flex items-center gap-1">
                    <span>{{ $stats['driver'] }}</span>
                </div>
            </div>
        </div>
    </div>
    ...
    {{-- 3. Tab Filter Bar --}}
    <div class="px-2 py-1 bg-white dark:bg-slate-900 rounded border border-slate-200/90 dark:border-slate-800 shadow-2xs flex items-center gap-1 select-none">
        <button type="button" @click="activeTab = 'backups'"
                class="px-2.5 py-1 rounded text-[11px] font-bold transition inline-flex items-center gap-1.5 shrink-0 cursor-pointer"
                :class="activeTab === 'backups' ? 'bg-violet-600 text-white shadow-2xs' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800'">
        ...
        <button type="button" @click="activeTab = 'restore'"
                class="px-2.5 py-1 rounded text-[11px] font-bold transition inline-flex items-center gap-1.5 shrink-0 cursor-pointer"
                :class="activeTab === 'restore' ? 'bg-amber-600 text-white shadow-2xs' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800'">
            <span>🔄 {{ __('messages.backup_restore_tab') }}</span>
        </button>
    </div>

    {{-- 4. Tab 1: Backups Archive List --}}
    <div x-show="activeTab === 'backups'" class="rounded bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 shadow-2xs overflow-hidden space-y-0">
        <div class="px-2 py-1.5 border-b border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1.5">
            ...
            <div class="flex items-center gap-1.5">
                <input type="text" x-model="searchQuery" placeholder="{{ __('messages.backup_search_placeholder') }}"
                       class="h-7 text-xs border border-slate-300 dark:border-slate-700 rounded px-2.5 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-slate-100 placeholder-slate-400 focus:outline-none focus:ring-1 focus:ring-violet-500 w-40 sm:w-48">

                <select x-model="formatFilter"
                        class="h-7 text-xs border border-slate-300 dark:border-slate-700 rounded px-2 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-1 focus:ring-violet-500">
                            ...
                            <td class="py-2.5 px-3.5 text-right">
                                <div class="inline-flex items-center gap-1.5">
                                    ...
                                    <a href="{{ route('store.admin.backups.download', array_merge($storeRouteParams, ['file' => $file['filename']])) }}"
                                       class="sf-btn-3d h-6 px-2.5 rounded-md text-xs font-semibold inline-flex items-center gap-1 cursor-pointer">
                                        ⬇ {{ __('messages.backup_download') }}
                                    </a>

                                    {{-- Restore from file button --}}
                                    <button type="button" @click="openRestoreModal('{{ $file['filename'] }}')"
                                             class="sf-btn-3d-gold h-6 px-2.5 rounded-md text-xs font-semibold inline-flex items-center gap-1 cursor-pointer">
                                    ...
                                    <form method="POST" action="{{ route('store.admin.backups.destroy', array_merge($storeRouteParams, ['file' => $file['filename']])) }}"
                                        ...
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="sf-btn-3d-danger h-6 px-2 rounded-md text-xs font-semibold inline-flex items-center gap-1 cursor-pointer">
    ...
    <div x-show="activeTab === 'restore'" class="rounded-2xl bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 p-4 sm:p-5 shadow-sm space-y-4">
        ...
        <form method="POST" action="{{ route('store.admin.backups.upload_restore', $storeRouteParams) }}" enctype="multipart/form-data"
            ...
            <button type="submit"
                    class="sf-btn-3d-gold py-2.5 px-5 rounded-md text-xs font-bold inline-flex items-center justify-center gap-2 cursor-pointer">
                <span>🔄</span>
        ...
        <div @click.away="restoreModalOpen = false"
            ...
            <form method="POST" action="{{ route('store.admin.backups.restore', $storeRouteParams) }}" class="flex items-center justify-end gap-2 pt-2">
                ...
                <button type="button" @click="restoreModalOpen = false"
                        class="sf-btn-3d px-4 py-2 text-xs font-bold rounded-md cursor-pointer">
                    {{ __('messages.backup_restore_cancel_btn') }}
                </button>
                <button type="submit"
                        class="sf-btn-3d-gold px-4 py-2 text-xs font-bold rounded-md cursor-pointer">
