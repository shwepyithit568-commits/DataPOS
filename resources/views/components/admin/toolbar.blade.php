...
<div x-data="{
...
}" class="rounded-3xl bg-white/95 dark:bg-slate-900/95 border border-slate-200/90 dark:border-slate-800 shadow-sm p-2.5 sm:p-3.5 mb-5 sm:mb-6 backdrop-blur-md transition">
    ...
    <div class="flex items-center gap-2 overflow-x-auto pb-1 pt-0.5 -mx-1 px-1 scrollbar-thin scrollbar-thumb-slate-300 dark:scrollbar-thumb-slate-700">
        ...
        <form method="GET" class="flex items-center gap-2 shrink-0" role="search">
            ...
            <div class="hidden sm:relative sm:flex items-center sm:w-56 md:w-64 lg:w-72" x-ref="searchFormDesktop">
                <input
                    ...
                    class="w-full pl-9 pr-9 py-2 min-h-[40px] border border-slate-200 dark:border-slate-700 rounded-2xl text-xs sm:text-sm bg-slate-50/80 dark:bg-slate-800/80 text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:bg-white dark:focus:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-violet-500/40 focus:border-violet-500 transition shadow-inner"
                />
                ...
                <svg x-show="searching" x-cloak class="w-4 h-4 text-violet-600 absolute right-3 animate-spin" fill="none" viewBox="0 0 24 24">
                ...
                @if ($hasActiveSearch)
                    <a href="{{ request()->url() . '?' . http_build_query(request()->except(['search', 'page'])) }}"
                       x-show="!searching"
                       class="absolute right-2.5 w-6 h-6 flex items-center justify-center text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 rounded-full hover:bg-slate-200 dark:hover:bg-slate-700 transition"
            ...
            <button type="button" @click="searchOpen = !searchOpen"
                class="sm:hidden shrink-0 min-h-[40px] min-w-[40px] flex items-center justify-center rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 transition relative shadow-sm {{ $hasActiveSearch ? 'ring-2 ring-violet-500 text-violet-600 dark:text-violet-400 bg-violet-50 dark:bg-violet-950/60' : '' }}"
        ...
        @if (count($filters) > 0)
            <button type="button" @click="showAdvanced = !showAdvanced"
                class="relative shrink-0 min-h-[40px] px-3.5 py-2 rounded-2xl text-xs sm:text-sm font-bold flex items-center gap-2 whitespace-nowrap transition border shadow-xs {{ count($activeFilters) > 0 ? 'bg-violet-50 text-violet-700 border-violet-200 dark:bg-violet-950/60 dark:text-violet-300 dark:border-violet-800' : 'bg-slate-50 hover:bg-slate-100 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 border-slate-200/80 dark:border-slate-700/80' }}">
                <svg class="w-4 h-4 {{ count($activeFilters) > 0 ? 'text-violet-600 dark:text-violet-400' : 'text-slate-500 dark:text-slate-400' }}" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z" />
                </svg>
                <span>{{ __('messages.filters') }}</span>
                @if (count($activeFilters) > 0)
                    <span class="min-w-[18px] h-[18px] px-1 flex items-center justify-center text-[10px] font-black bg-violet-600 text-white rounded-full shadow-xs">
                        {{ count($activeFilters) }}
                    </span>
                @endif
                <svg class="w-3.5 h-3.5 text-slate-400 transition-transform duration-200" :class="showAdvanced ? 'rotate-180 text-violet-600 dark:text-violet-400' : ''" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <form method="GET" class="hidden sm:block shrink-0">
            ...
            <div class="relative">
                ...
                <select name="sort" data-auto-submit
                    class="border border-slate-200 dark:border-slate-700 rounded-2xl pl-9 pr-8 min-h-[40px] py-1.5 text-xs sm:text-sm bg-slate-50/80 dark:bg-slate-800/80 text-slate-700 dark:text-slate-200 font-bold focus:outline-none focus:ring-2 focus:ring-violet-500/40 cursor-pointer appearance-none shadow-xs transition">
        ...
        @if ($showViewToggle)
            <div class="flex items-center bg-slate-100 dark:bg-slate-800/90 p-1 rounded-2xl border border-slate-200/80 dark:border-slate-700 shrink-0 shadow-inner">
                <button type="button"
                    @click="$dispatch('view-changed', 'table'); currentView = 'table'; localStorage.setItem('admin_view_mode', 'table')"
                    :class="currentView === 'table' ? 'bg-white dark:bg-slate-700 shadow-sm text-violet-600 dark:text-violet-400 font-black' : 'text-slate-500 hover:text-slate-900 dark:hover:text-white'"
                    class="px-2.5 py-1.5 rounded-xl text-xs font-bold transition flex items-center gap-1.5"
                ...
                <button type="button"
                    ...
                    :class="currentView === 'card' ? 'bg-white dark:bg-slate-700 shadow-sm text-violet-600 dark:text-violet-400 font-black' : 'text-slate-500 hover:text-slate-900 dark:hover:text-white'"
                    class="px-2.5 py-1.5 rounded-xl text-xs font-bold transition flex items-center gap-1.5"
        ...
        @if ($bulkActions)
            <button type="button" @click="$dispatch('bulk-actions-request')"
                class="relative shrink-0 min-h-[40px] px-3.5 py-2 bg-slate-50 hover:bg-slate-100 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 rounded-2xl text-xs sm:text-sm font-bold flex items-center gap-1.5 whitespace-nowrap transition border border-slate-200/80 dark:border-slate-700 shadow-xs">
        ...
        @if ($showExportImport)
            @if ($importUrl)
                <a href="{{ $importUrl }}" class="shrink-0 min-h-[40px] px-3.5 py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-2xl text-xs sm:text-sm font-bold shadow-xs flex items-center gap-1.5 transition active:scale-95">
                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            @endif
            @if ($exportUrl)
                <a href="{{ $exportUrl }}" class="shrink-0 min-h-[40px] px-3.5 py-2 bg-violet-600 hover:bg-violet-500 text-white rounded-2xl text-xs sm:text-sm font-bold shadow-xs flex items-center gap-1.5 transition active:scale-95">
                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        @if ($showPerPageSelector)
            <form method="GET" class="shrink-0 flex items-center gap-1 bg-slate-100 dark:bg-slate-800 rounded-2xl border border-slate-200/80 dark:border-slate-700 p-1 shadow-inner">
                ...
                <select name="per_page" data-auto-submit title="{{ __('messages.items_per_page') }}"
                    class="border border-slate-200 dark:border-slate-700 rounded-xl px-2 min-h-[30px] py-1 text-xs bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-200 font-bold focus:outline-none focus:ring-2 focus:ring-violet-500/40 cursor-pointer shadow-xs">
                ...
                <span class="text-[11px] font-semibold text-slate-400 dark:text-slate-500 pr-1.5 hidden sm:inline">/ {{ __('messages.items_per_page') }}</span>
            </form>
        @endif

        {{-- ===== 8. RESULT COUNT BADGE ===== --}}
        @if ($totalCount !== null)
            <span class="shrink-0 ml-auto inline-flex items-center px-3 py-1.5 rounded-2xl bg-slate-100 dark:bg-slate-800/80 border border-slate-200/70 dark:border-slate-700 text-xs font-black text-slate-600 dark:text-slate-300 font-mono whitespace-nowrap shadow-inner">
    ...
    @if (count($filters) > 0 || count($sortOptions) > 0)
        <div x-show="showAdvanced" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 -translate-y-2" x-transition:enter-end="opacity-100 translate-y-0" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100 translate-y-0" x-transition:leave-end="opacity-0 -translate-y-2" class="pt-3.5 mt-3 border-t border-slate-100 dark:border-slate-800 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4 p-4 rounded-3xl bg-slate-50/80 dark:bg-slate-800/60 border border-slate-200/80 dark:border-slate-700/80">
            {{-- Mobile-only: Sort --}}
            @if (count($sortOptions) > 0)
                <div class="sm:hidden">
