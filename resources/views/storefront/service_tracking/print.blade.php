@php
    ...
    $paperSize = in_array($paperSize, ['58mm', '80mm', 'a5', 'a4'], true) ? $paperSize : 'a5';
@endphp
<!DOCTYPE html>
...
<head>
    ...
    <title>{{ __('messages.repair_ticket_heading') }} {{ $repair->voucher_no ?? $repair->job_number }} — {{ $store->name }}</title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /* Share Modal (Mobile-first Action Sheet) */
        .share-modal {
            display: none;
            position: fixed;
            inset: 0;
...
<body>
    ...
    <div class="toolbar-wrap no-print">
        <div class="toolbar-left">
            <a href="{{ route('storefront.service.track.token', ['store_slug' => $store->slug, 'token' => $repair->tracking_token]) }}"
            ...
            <div class="size-pills">
                <a href="{{ route('storefront.service.track.print', ['store_slug' => $store->slug, 'token' => $repair->tracking_token, 'paper_size' => '58mm']) }}"
                   class="size-pill {{ $paperSize === '58mm' ? 'active' : '' }}" title="58mm POS Slip">
                    58mm
                </a>
                <a href="{{ route('storefront.service.track.print', ['store_slug' => $store->slug, 'token' => $repair->tracking_token, 'paper_size' => '80mm']) }}"
                   class="size-pill {{ $paperSize === '80mm' ? 'active' : '' }}" title="80mm POS Thermal">
                    80mm
                </a>
                <a href="{{ route('storefront.service.track.print', ['store_slug' => $store->slug, 'token' => $repair->tracking_token, 'paper_size' => 'a5']) }}"
                   class="size-pill {{ $paperSize === 'a5' ? 'active' : '' }}" title="A5 Half Sheet">
                    A5
                </a>
                <a href="{{ route('storefront.service.track.print', ['store_slug' => $store->slug, 'token' => $repair->tracking_token, 'paper_size' => 'a4']) }}"
        ...
        <div class="toolbar-right">
            ...
            <button type="button" class="tool-btn tool-btn-pdf" id="btnDownloadPdf" data-repair-download-pdf>
                📥 <span>{{ __('messages.track_service_save_pdf') }}</span>
            </button>
    ...
    <div class="ticket-wrapper">
        <div class="ticket-card size-{{ $paperSize }}" id="ticketDocument">
            ...
            <div class="job-title-row">
                ...
                <div class="job-number">{{ $repair->voucher_no ?? $repair->job_number }}</div>
                <div>
            ...
            <div style="text-align:center; font-size:0.72em; color:#94a3b8; margin-top:2px;">
                {{ __('messages.repair_printed_date') }}: {{ now()->format('d/m/Y H:i') }} · {{ $store->name }}
            </div>
    ...
    <script nonce="{{ $cspNonce ?? '' }}">
        ...
        var pdfPaperSize = 'a5';
        var jobNumber = @js($repair->voucher_no ?? $repair->job_number);
                ...
                try {
                    ...
                    if (navigator.canShare && navigator.canShare({ files: [pdfFile] })) {
                        await navigator.share({
                            title: 'Service Slip #' + jobNumber,
                            text: storeName + ' - Service Slip #' + jobNumber + '\nTracking: ' + trackingUrl,
                            files: [pdfFile]
                        });
                        return;
                    }
        ...
        function buildShareText() {
            var text = 'မင်္ဂလာပါ ' + @js($customerName) + ' ခင်ဗျာ။\n\n'
                + storeName + ' မှ ပြေစာအချက်အလက် ဖြစ်ပါသည် -\n'
                + 'စက်ပြင်နံပါတ်: #' + jobNumber + '\n'
                + 'စက်အမျိုးအစား: ' + deviceLabel + '\n';
            if (imeiSerial) text += 'IMEI/Serial: ' + imeiSerial + '\n';
            text += 'လက်ခံရက်စွဲ: ' + dateLabel + '\n'
                + 'ကျသင့်ငွေ: ' + chargeLabel + '\n'
                + 'ပေးသွင်းပြီး: ' + paidLabel + '\n';
            if (hasOutstanding) text += 'ကျန်ငွေ: ' + outstandingLabel + '\n';
            text += '\nအခြေအနေစစ်ဆေးရန်: ' + trackingUrl;
