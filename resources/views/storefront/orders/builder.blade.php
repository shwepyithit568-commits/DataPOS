...
<div class="w-full max-w-5xl mx-auto space-y-1 sm:space-y-2 pb-16 sm:pb-12 select-none">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2 sm:gap-3 px-2 sm:px-3 py-1.5 sm:py-2 border border-slate-200/80 dark:border-slate-800 bg-white dark:bg-slate-900 rounded-lg sm:rounded-xl shadow-2xs">
        ...
        <a href="{{ url('/products?store_slug=' . ($store?->slug ?? request('store_slug'))) }}"
           class="sf-btn-3d self-start sm:self-auto !inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-bold shrink-0">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-1 sm:gap-1.5 lg:gap-2 items-start">
        ...
        <div class="lg:col-span-7 space-y-1 sm:space-y-1.5">
            <div class="bg-white dark:bg-slate-900 rounded-lg sm:rounded-xl p-2 sm:p-3.5 lg:p-4 border border-slate-200/90 dark:border-slate-800 shadow-xs space-y-2 sm:space-y-3">
                <div class="flex items-center justify-between border-b border-slate-100 dark:border-slate-800/80 pb-2">
                    <h2 class="font-black text-xs sm:text-sm text-slate-900 dark:text-white flex items-center gap-1.5">
                        <span class="inline-flex h-5 w-5 sm:h-6 sm:w-6 items-center justify-center rounded bg-slate-100 dark:bg-slate-800 text-xs">🛒</span>
                        <span>{{ __('messages.selected_products') }}</span>
                    </h2>
                    <span class="text-[10px] sm:text-xs font-black text-[color:var(--sf-primary)] dark:text-[color:var(--sf-primary-hover)] bg-slate-50 dark:bg-slate-800 px-2 py-0.5 rounded-md border border-slate-200/80 dark:border-slate-700/80"
                ...
                <template x-if="!$store.orderBuilder || $store.orderBuilder.items.length === 0">
                    <div class="text-center py-8 sm:py-10 space-y-2.5 bg-slate-50/50 dark:bg-slate-800/30 rounded-lg border border-dashed border-slate-200 dark:border-slate-700/80 p-4">
                        <div class="w-12 h-12 sm:w-14 sm:h-14 mx-auto rounded-md bg-white dark:bg-slate-800 flex items-center justify-center text-2xl sm:text-3xl shadow-2xs border border-slate-200/80 dark:border-slate-700">🛍️</div>
                        ...
                        <a href="{{ url('/products?store_slug=' . ($store?->slug ?? request('store_slug'))) }}"
                           class="sf-btn-3d-primary !inline-flex items-center gap-1.5 px-4 py-2 rounded-md font-black text-xs shadow-xs">
                            <span>{{ __('messages.view_products') }}</span>
                            <span aria-hidden="true">→</span>
                        </a>
                    </div>
                    ...
                    <template x-for="item in ($store.orderBuilder ? $store.orderBuilder.items : [])" :key="item.id">
                        <div class="pt-1.5 sm:pt-2 first:pt-0 flex items-center justify-between gap-2 sm:gap-3">
                            <div class="flex items-center space-x-2 sm:space-x-2.5 flex-1 min-w-0">
                                <div class="w-12 h-12 sm:w-14 sm:h-14 rounded-md bg-slate-100 dark:bg-slate-800 shrink-0 overflow-hidden border border-slate-200/80 dark:border-slate-700 flex items-center justify-center">
                            ...
                            <div class="flex items-center space-x-1 sm:space-x-1.5 shrink-0">
                                <div class="flex items-center border border-slate-200 dark:border-slate-700 rounded-md bg-slate-50 dark:bg-slate-800/80 p-0.5 shadow-2xs">
                                    <button @click="$store.orderBuilder.updateQty(item.id, -1)"
                                            type="button"
                                            class="sf-btn-3d w-7 h-7 sm:w-8 sm:h-8 rounded-sm !flex items-center justify-center font-black text-xs sm:text-sm text-slate-700 dark:text-slate-200 transition cursor-pointer select-none"
                                ...
                                <button @click="$store.orderBuilder.removeItem(item.id)"
                                        type="button"
                                        class="sf-btn-3d-danger w-7 h-7 sm:w-8 sm:h-8 rounded-md !flex items-center justify-center text-white transition cursor-pointer shadow-2xs"
                ...
                <div class="pt-2 sm:pt-3 border-t border-slate-100 dark:border-slate-800/80 space-y-1.5 text-xs sm:text-sm">
                    @if ($taxEnabled && $taxType === 'exclusive')
                        ...
                        <div class="flex items-center justify-between text-slate-600 dark:text-slate-400">
                            ...
                            <span class="font-mono font-bold"
                                  x-text="'+ ' + (typeof window.formatCurrency === 'function' ? window.formatCurrency(Math.round(($store.orderBuilder ? $store.orderBuilder.taxableAmount : 0) * {{ $taxRate }} / 100)) : Math.round(($store.orderBuilder ? $store.orderBuilder.taxableAmount : 0) * {{ $taxRate }} / 100).toLocaleString())"></span>
                        </div>
                    @endif
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-1 sm:gap-1.5 text-center text-xs">
                <div class="p-2 sm:p-2.5 bg-white dark:bg-slate-900 rounded-lg border border-slate-200/80 dark:border-slate-800 shadow-2xs flex flex-col items-center justify-center">
        ...
        <div class="lg:col-span-5 space-y-1 sm:space-y-1.5">
            <div class="bg-white dark:bg-slate-900 rounded-lg sm:rounded-xl p-2 sm:p-3.5 lg:p-4 border border-slate-200/90 dark:border-slate-800 shadow-xs space-y-2 sm:space-y-3 sticky top-16 sm:top-20"
                 x-data="{
                ...
                <form x-ref="orderForm" method="POST" action="{{ url('/store/' . ($store?->slug ?? request('store_slug')) . '/orders') }}" @submit.prevent="submitOrder($event)" class="space-y-2.5 sm:space-y-3">
                    ...
                    <div>
                        ...
                        <input
                            ...
                            class="w-full px-3 py-2 rounded-md border border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-xs font-bold text-slate-900 dark:text-white focus:ring-1 focus:ring-[color:var(--sf-primary)] focus:border-[color:var(--sf-primary)] outline-none transition"
                        />
                        @error('customer_name')
                    ...
                    <div>
                        ...
                        <textarea
                            ...
                            class="w-full px-3 py-1.5 rounded-md border border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-xs font-medium text-slate-900 dark:text-white focus:ring-1 focus:ring-[color:var(--sf-primary)] focus:border-[color:var(--sf-primary)] outline-none transition"
                    ...
                    <div class="space-y-1" x-show="contactChannel === 'viber' || contactChannel === 'telegram'" x-transition>
                        ...
                        <input
                            ...
                            class="w-full rounded-md border border-slate-300 dark:border-slate-700 px-3 py-1.5 bg-slate-50/50 dark:bg-slate-800 text-slate-900 dark:text-white text-xs font-bold focus:ring-1 focus:ring-[color:var(--sf-primary)] focus:border-[color:var(--sf-primary)] focus:outline-none shadow-2xs"
                            autocomplete="tel"
                        />
                    </div>
                    ...
                    <div class="pt-1.5 border-t border-slate-100 dark:border-slate-800/80">
                        ...
                        <div class="grid grid-cols-2 gap-1.5 sm:gap-2">
                            ...
                            <button type="button" @click="contactChannel = 'viber'"
                                class="sf-btn-3d-viber !flex items-center justify-center gap-1.5 py-2.5 sm:py-3 px-2 rounded-md text-xs font-black select-none transition cursor-pointer"
                                ...
                                <span class="inline-flex items-center justify-center w-4 h-4 sm:w-5 sm:h-5 rounded-md bg-white/20 shrink-0">
                            ...
                            <button type="button" @click="contactChannel = 'telegram'"
                                class="sf-btn-3d-telegram !flex items-center justify-center gap-1.5 py-2.5 sm:py-3 px-2 rounded-md text-xs font-black select-none transition cursor-pointer"
                            ...
                            <button type="button" @click="contactChannel = 'phone'"
                                class="sf-btn-3d-success !flex items-center justify-center gap-1.5 py-2.5 sm:py-3 px-2 rounded-md text-xs font-black select-none transition cursor-pointer"
                                ...
                                <span class="inline-flex items-center justify-center w-4 h-4 sm:w-5 sm:h-5 rounded-md bg-white/20 shrink-0 text-xs" aria-hidden="true">📞</span>
                                <span class="font-black text-white">Phone</span>
                            </button>

                            {{-- 4. Submit (Send Order) — sf-btn-3d-gold --}}
                            <button
                                type="submit"
                                :disabled="!$store.orderBuilder || $store.orderBuilder.items.length === 0"
                                class="sf-btn-3d-gold !flex items-center justify-center gap-1.5 py-2.5 sm:py-3 px-2 rounded-md text-xs font-black disabled:opacity-50 disabled:pointer-events-none select-none cursor-pointer"
                            >
    ...
    @if ($obHasDelivery || $obHasPayment)
        <div class="bg-white dark:bg-slate-900 rounded-lg sm:rounded-xl p-2 sm:p-3.5 lg:p-4 border border-slate-200/90 dark:border-slate-800 shadow-xs">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-2 sm:gap-3">
                @if ($obHasDelivery)
                    <div class="space-y-1.5">
                        ...
                        @if ($obDeliveries->isNotEmpty())
                            <ul class="space-y-1">
                                @foreach ($obDeliveries as $dm)
                                    <li class="rounded-md bg-slate-50 dark:bg-slate-800/60 p-2 border border-slate-100 dark:border-slate-700/60">
                        ...
                        @else
                            <p class="text-xs text-slate-600 dark:text-slate-300 leading-relaxed whitespace-pre-line bg-slate-50 dark:bg-slate-800/60 p-2 rounded-md">{{ $store->setting->delivery_info }}</p>
                        @endif
                    </div>
                @endif
                @if ($obHasPayment)
                    <div class="space-y-1.5">
                                ...
                                @foreach ($obPayments as $pm)
                                    <li>
                                        <button type="button"
                                            ...
                                            class="w-full flex items-center justify-between gap-2 rounded-md bg-white dark:bg-slate-800/70 p-2 border border-slate-200/80 dark:border-slate-700/60 hover:border-slate-400 dark:hover:border-slate-500 transition group cursor-pointer text-left">
                                            ...
                                            <span class="shrink-0 px-1.5 py-0.5 rounded-md bg-slate-100 group-hover:bg-slate-200 dark:bg-slate-700 dark:group-hover:bg-slate-600 border border-slate-200 dark:border-slate-600 text-slate-700 dark:text-slate-200 text-[10px] font-bold transition flex items-center gap-1">
                        ...
                        @else
                            <p class="text-xs text-slate-600 dark:text-slate-300 leading-relaxed whitespace-pre-line bg-slate-50 dark:bg-slate-800/60 p-2 rounded-md">{{ $store->setting->payment_info }}</p>
                        @endif
                    </div>
                @endif
            </div>
